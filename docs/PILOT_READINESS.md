...
Pilot burn-in milestone is marked complete for the local/staging-shaped burn-in evidence only.
`docs/PILOT_BURNIN_REPORT.md` records repeated Alpaca follow-up attempts that failed before lawful LIVE liquidity because credentials were unavailable; those attempts are not valid vendor freshness, off-hours, or symbol-parity evidence.

Do not claim credentialed Alpaca/provider readiness until a run produced on a host with working provider credentials is archived and linked from this section.
