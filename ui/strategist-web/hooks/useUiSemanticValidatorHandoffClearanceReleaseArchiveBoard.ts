...
import type { UiSemanticValidatorHandoffClearanceReleaseArchiveBoardPayload } from "@/lib/api/types";

type Nullable<T> = T | null | undefined;

export type UiSemanticValidatorHandoffClearanceReleaseArchiveBoardQuery = {
...
function path(q?: UiSemanticValidatorHandoffClearanceReleaseArchiveBoardQuery) {
  const p = new URLSearchParams();
  if (q) {
    append(p, "experiment_id_contains", q.experimentIdContains);
    append(p, "issue_contains", q.issueContains);
    append(p, "ready_for_human_archive_observation", q.readyForHumanArchiveObservation);
    ...
    append(p, "requires_release_closure_review", q.requiresReleaseClosureReview);
  ...
  const qs = p.toString();
  return qs
    ? `/ui/semantic-validator-handoff/clearance-release-archive-board?${qs}`
    : "/ui/semantic-validator-handoff/clearance-release-archive-board";
}

export function useUiSemanticValidatorHandoffClearanceReleaseArchiveBoard(
  query?: UiSemanticValidatorHandoffClearanceReleaseArchiveBoardQuery,
) {
  return useReadPlaneJsonQuery<UiSemanticValidatorHandoffClearanceReleaseArchiveBoardPayload>(
    queryKeys.uiSemanticValidatorHandoffClearanceReleaseArchiveBoardFiltered(query ?? {}),
    path(query),
  );
}
