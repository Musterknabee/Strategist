...
import type { UiSemanticValidatorHandoffClearanceReleaseClosureBoardPayload } from "@/lib/api/types";

type Nullable<T> = T | null | undefined;

export type UiSemanticValidatorHandoffClearanceReleaseClosureBoardQuery = {
  experimentIdContains?: Nullable<string>;
  issueContains?: Nullable<string>;
  evidenceLane?: Nullable<string>;
  releaseClosureStatus?: Nullable<string>;
  releaseClosureReadiness?: Nullable<string>;
...
function path(q?: UiSemanticValidatorHandoffClearanceReleaseClosureBoardQuery) {
  const p = new URLSearchParams();
  if (q) {
    append(p, "experiment_id_contains", q.experimentIdContains);
    append(p, "issue_contains", q.issueContains);
    append(p, "ready_for_human_closure_observation", q.readyForHumanClosureObservation);
    ...
    append(p, "requires_release_completion_review", q.requiresReleaseCompletionReview);
    append(p, "limit", q.limit);
    if (q.evidenceLane) p.append("evidence_lane", q.evidenceLane);
    if (q.releaseClosureStatus) p.append("release_closure_status", q.releaseClosureStatus);
    if (q.releaseClosureReadiness) p.append("release_closure_readiness", q.releaseClosureReadiness);
  ...
  const qs = p.toString();
  return qs
    ? `/ui/semantic-validator-handoff/clearance-release-closure-board?${qs}`
    : "/ui/semantic-validator-handoff/clearance-release-closure-board";
}

export function useUiSemanticValidatorHandoffClearanceReleaseClosureBoard(
  query?: UiSemanticValidatorHandoffClearanceReleaseClosureBoardQuery,
) {
  return useReadPlaneJsonQuery<UiSemanticValidatorHandoffClearanceReleaseClosureBoardPayload>(
    queryKeys.uiSemanticValidatorHandoffClearanceReleaseClosureBoardFiltered(query ?? {}),
    path(query),
  );
}
