...
import type { UiSemanticValidatorHandoffClearanceReleaseAcknowledgmentBoardPayload } from "@/lib/api/types";

type Nullable<T> = T | null | undefined;

export type UiSemanticValidatorHandoffClearanceReleaseAcknowledgmentBoardQuery = {
...
function path(q?: UiSemanticValidatorHandoffClearanceReleaseAcknowledgmentBoardQuery) {
  const p = new URLSearchParams();
  if (q) {
    append(p, "experiment_id_contains", q.experimentIdContains);
    append(p, "issue_contains", q.issueContains);
    append(p, "ready_for_human_acknowledgment_observation", q.readyForHumanAcknowledgmentObservation);
    ...
    append(p, "requires_release_receipt_review", q.requiresReleaseReceiptReview);
  ...
  const qs = p.toString();
  return qs
    ? `/ui/semantic-validator-handoff/clearance-release-acknowledgment-board?${qs}`
    : "/ui/semantic-validator-handoff/clearance-release-acknowledgment-board";
}

export function useUiSemanticValidatorHandoffClearanceReleaseAcknowledgmentBoard(
  query?: UiSemanticValidatorHandoffClearanceReleaseAcknowledgmentBoardQuery,
) {
  return useReadPlaneJsonQuery<UiSemanticValidatorHandoffClearanceReleaseAcknowledgmentBoardPayload>(
    queryKeys.uiSemanticValidatorHandoffClearanceReleaseAcknowledgmentBoardFiltered(query ?? {}),
    path(query),
  );
}
