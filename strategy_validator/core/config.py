...
def load_config(config_path: str | Path | None = None) -> AppConfig:
    ...
    if openbb is not None:
        if os.environ.get("STRATEGY_VALIDATOR_OPENBB_ENABLED") is not None:
            openbb.enabled = _env_bool("STRATEGY_VALIDATOR_OPENBB_ENABLED", openbb.enabled)
        ...
        if os.environ.get("STRATEGY_VALIDATOR_OPENBB_MODE"):
            openbb.mode = os.environ["STRATEGY_VALIDATOR_OPENBB_MODE"].strip().lower()
        if os.environ.get("STRATEGY_VALIDATOR_OPENBB_BASE_URL"):
            openbb.base_url = os.environ["STRATEGY_VALIDATOR_OPENBB_BASE_URL"].strip() or None
        if os.environ.get("STRATEGY_VALIDATOR_OPENBB_LIQUIDITY_URL_TEMPLATE"):
            openbb.liquidity_url_template = os.environ["STRATEGY_VALIDATOR_OPENBB_LIQUIDITY_URL_TEMPLATE"].strip()
        if os.environ.get("STRATEGY_VALIDATOR_OPENBB_BORROW_URL_TEMPLATE"):
            openbb.borrow_url_template = os.environ["STRATEGY_VALIDATOR_OPENBB_BORROW_URL_TEMPLATE"].strip()
        ...
        if os.environ.get("STRATEGY_VALIDATOR_OPENBB_TIMEOUT_SECONDS") is not None:
            try:
                openbb.timeout_seconds = float(os.environ["STRATEGY_VALIDATOR_OPENBB_TIMEOUT_SECONDS"])
            except ValueError:
                pass
        if os.environ.get("STRATEGY_VALIDATOR_OPENBB_SOURCE_MODE"):
            openbb.source_mode = os.environ["STRATEGY_VALIDATOR_OPENBB_SOURCE_MODE"].strip().upper()
    ...
    if nim is not None:
        if os.environ.get("STRATEGY_VALIDATOR_NVIDIA_NIM_ENABLED") is not None:
            nim.enabled = _env_bool("STRATEGY_VALIDATOR_NVIDIA_NIM_ENABLED", nim.enabled)
        ...
        if os.environ.get("STRATEGY_VALIDATOR_NVIDIA_NIM_BASE_URL"):
            nim.base_url = os.environ["STRATEGY_VALIDATOR_NVIDIA_NIM_BASE_URL"].strip()
        ...
        if os.environ.get("STRATEGY_VALIDATOR_NVIDIA_NIM_TIMEOUT_SECONDS") is not None:
            try:
                nim.timeout_seconds = float(os.environ["STRATEGY_VALIDATOR_NVIDIA_NIM_TIMEOUT_SECONDS"])
            except ValueError:
                pass
        if os.environ.get("STRATEGY_VALIDATOR_NVIDIA_NIM_MAX_RETRIES") is not None:
            try:
                nim.max_retries = int(os.environ["STRATEGY_VALIDATOR_NVIDIA_NIM_MAX_RETRIES"])
            except ValueError:
                pass
