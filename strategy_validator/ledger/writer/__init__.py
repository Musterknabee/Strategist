...
def commit_state_transition(
    ...
    *,
    ...
    staged_evidence_items = getattr(experiment, "_ledger_evidence_items", None)
    if staged_evidence_items is not None:
        payload.setdefault("evidence_bundle", {})
        if isinstance(payload["evidence_bundle"], dict):
            normalized_items: list[dict[str, object]] = []
            for item in tuple(staged_evidence_items):
                if hasattr(item, "model_dump"):
                    normalized_items.append(item.model_dump(mode="json"))
                elif isinstance(item, dict):
                    normalized_items.append(dict(item))
            payload["evidence_bundle"]["evidence_items"] = normalized_items
