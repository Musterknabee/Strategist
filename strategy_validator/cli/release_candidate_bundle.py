...
def cmd_generate(candidate: str | None) -> Path:
    ...
    from strategy_validator.cli.release_candidate_common import _safe_candidate_id
...
def cmd_verify_bundle(candidate: str) -> dict[str, Any]:
    ...
    if all(isinstance(entry, dict) for entry in entries):
        try:
            recomputed_content_sha256 = _bundle_entries_content_sha256(entries)  # type: ignore[arg-type]
