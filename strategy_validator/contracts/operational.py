...
class DeploymentReadiness(BaseModel):
    ...
    mutation_safety: MutationSafetyStatus
    adjudication_allowed: bool
    checks: Dict[str, bool] = {}

    model_config = {"extra": "forbid"}

class MutationSafetyStatus(BaseModel):
    """Explicit runtime status for operator/API mutation protection."""
    runtime_mode: RuntimeMode
    authorization_mode: Literal["TOKEN_PROTECTED", "NON_PRODUCTION_BYPASS", "MISCONFIGURED"]
    token_configured: bool
    mutation_routes_safe: bool
    detail_code: str
...
class DeploymentReadinessTierReport(BaseModel):
    """Operator deployment preflight beyond runtime adjudication readiness."""
    status: Literal["READY", "DEGRADED", "BLOCKED"]
    checked_at_utc: datetime
    runtime_readiness_status: Literal["READY", "DEGRADED", "BLOCKED"]
    config_fingerprint: str
    blockers: List[ReadinessBlocker] = []
    warnings: List[ReadinessBlocker] = []
    checks: Dict[str, bool] = {}
    ledger_database_path: Optional[str] = None
    ledger_backup_dir: Optional[str] = None
    private_key_file_count: int = 0
    scanned_key_file_count: int = 0
