"""Read-only clearance closeout board for semantic validator handoff closure.

This module is intentionally kept as a compatibility facade. The clearance
closeout-board implementation is decomposed into focused subphase modules for
shared helpers, row synthesis/filtering, and public payload assembly.
"""
from __future__ import annotations

from strategy_validator.application.ui_semantic_validator_handoff_clearance_closeout_board_common import (
    _LIMIT_MAX,
    _PRIORITY_RANK,
    _READINESS_RANK,
    _SCHEMA_VERSION,
    _SEVERITY_RANK,
    _STATUS_RANK,
    _as_list,
    _authority,
    _best_priority,
    _best_severity,
    _contains,
    _counts,
    _digest,
    _norm,
    _norm_set,
    _s,
    _uniq,
    _utc_now,
)
from strategy_validator.application.ui_semantic_validator_handoff_clearance_closeout_board_payload import (
    build_ui_semantic_validator_handoff_clearance_closeout_board_latest_payload,
    build_ui_semantic_validator_handoff_clearance_closeout_board_payload,
)
from strategy_validator.application.ui_semantic_validator_handoff_clearance_closeout_board_rows import (
    _card,
    _closeout_gate,
    _closeout_note,
    _closeout_status,
    _degraded,
    _haystack,
    _matches,
    _readiness,
    _sort_card,
)
from strategy_validator.application.ui_semantic_validator_handoff_clearance_verification_board import (
    build_ui_semantic_validator_handoff_clearance_verification_board_payload,
)

__all__ = [
    "build_ui_semantic_validator_handoff_clearance_closeout_board_payload",
    "build_ui_semantic_validator_handoff_clearance_closeout_board_latest_payload",
]
