"""Public payload builders for semantic validator handoff clearance review docket."""
...
def _source_builder() -> SourceBuilder:
    ...
    return facade.build_ui_semantic_validator_handoff_clearance_closeout_board_payload


def build_ui_semantic_validator_handoff_clearance_review_docket_payload(
    *,
    ...
    source_cards = [row for row in source_payload.get("closeout_cards", []) if isinstance(row, dict)]
    rows = sorted(
        [_docket_from_card(card, index, source_payload) for index, card in enumerate(source_cards, start=1)],
        key=_sort_docket,
    )
    filtered = [
        row
        ...
        if _matches(
            ...
            closeout_readiness=_norm_set(closeout_readiness),
            priority=_norm_set(priority),
            severity=_norm_set(severity),
            trust_banner=_norm_set(trust_banner),
            owner_hint=_norm_set(owner_hint),
            ready_for_authorized_review=ready_for_authorized_review,
            blocked=blocked,
            waiting=waiting,
            requires_authorized_human_review=requires_authorized_human_review,
        )
    ]
    ...
    return {
        ...
        "filters": {
            ...
            "closeout_readiness": list(closeout_readiness or ()),
        ...
        "summary": {
            "source_closeout_card_count_total": len(source_cards),
            "review_docket_count_total": len(rows),
            "review_docket_count_filtered": len(filtered),
            "review_docket_count_returned": len(returned),
            "fail_closed_count": sum(1 for row in filtered if row.get("docket_readiness") == "FAIL_CLOSED"),
            "waiting_count": sum(1 for row in filtered if row.get("docket_readiness") == "WAITING"),
            "authorized_review_observation_count": sum(1 for row in filtered if row.get("docket_readiness") == "AUTHORIZED_REVIEW_OBSERVATION"),
            "ready_for_authorized_review_count": sum(1 for row in filtered if row.get("ready_for_authorized_review")),
            "blocked_count": sum(1 for row in filtered if row.get("blocked")),
            "requires_authorized_human_review_count": sum(1 for row in filtered if row.get("requires_authorized_human_review")),
            ...
            "latest_review_docket_id": None if latest is None else latest.get("review_docket_id"),
        },
        "docket_status_counts": _counts(filtered, "docket_status"),
        "docket_readiness_counts": _counts(filtered, "docket_readiness"),
        "source_closeout_status_counts": _counts(filtered, "source_closeout_status"),
        "source_closeout_readiness_counts": _counts(filtered, "source_closeout_readiness"),
        ...
        "guardrails": [
            "read_plane_only_clearance_review_docket_no_review_record_authorization_approval_signoff_or_decision_is_written_here",
            "review_docket_rows_are_observations_derived_from_clearance_closeout_cards_not_operator_actions",
            "ready_for_authorized_review_only_means_closeout_sources_observe_review_readiness",
        ...
        "latest": latest,
        "review_dockets": returned,
    }


def build_ui_semantic_validator_handoff_clearance_review_docket_latest_payload(
