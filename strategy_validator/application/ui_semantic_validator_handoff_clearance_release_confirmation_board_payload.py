"""Public payload builders for semantic validator handoff clearance release confirmation board."""
...
from strategy_validator.application.ui_semantic_validator_handoff_clearance_release_confirmation_board_rows import (
    _degraded,
    _matches,
    _row,
    _sort,
...
def _source_builder() -> SourceBuilder:
    ...
    from strategy_validator.application import ui_semantic_validator_handoff_clearance_release_confirmation_board as facade

    return facade.build_ui_semantic_validator_handoff_clearance_release_acknowledgment_board_payload


def build_ui_semantic_validator_handoff_clearance_release_confirmation_board_payload(
    *,
    ...
    kw = dict(
        ...
        ready_for_human_confirmation_observation=ready_for_human_confirmation_observation,
        blocked=blocked,
        waiting=waiting,
        requires_acceptance_observation=requires_acceptance_observation,
        requires_external_artifact=requires_external_artifact,
        requires_release_acknowledgment_review=requires_release_acknowledgment_review,
    )
    ...
    summary = {
        "source_release_acknowledgment_count_total": len(source_rows),
        "release_confirmation_count_total": len(rows),
        "release_confirmation_count_filtered": len(filtered),
        "release_confirmation_count_returned": len(returned),
        "fail_closed_count": sum(1 for row in filtered if row.get("release_confirmation_readiness") == "FAIL_CLOSED"),
        "waiting_count": sum(1 for row in filtered if row.get("release_confirmation_readiness") == "WAITING"),
        "human_confirmation_ready_observation_count": sum(
            1
            for row in filtered
            if row.get("release_confirmation_readiness") == "HUMAN_CONFIRMATION_READY_OBSERVATION"
        ),
        "ready_for_human_confirmation_observation_count": sum(
            1 for row in filtered if row.get("ready_for_human_confirmation_observation")
        ),
        ...
        "requires_release_acknowledgment_review_count": sum(1 for row in filtered if row.get("requires_release_acknowledgment_review")),
        ...
        "release_acknowledgment_assertion_allowed_count": 0,
        ...
        "latest_release_confirmation_id": None if latest is None else latest.get("release_confirmation_id"),
    }
    return {
        ...
        "filters": {},
        "summary": summary,
        "release_confirmation_status_counts": _counts(filtered, "release_confirmation_status"),
        "release_confirmation_readiness_counts": _counts(filtered, "release_confirmation_readiness"),
        ...
        "guardrails": [
            "read_plane_only_clearance_release_confirmation_no_confirmation_acknowledgment_receipt_custody_transfer_handoff_packet_release_acceptance_signoff_approval_or_decision_is_written_here",
            "confirmation_cards_are_observations_derived_from_release_acknowledgments_not_operator_actions",
        ...
        "routes": {
            ...
            "clearance_release_receipt_board": "/ui/semantic-validator-handoff/clearance-release-receipt-board",
        },
        "latest": latest,
        "release_confirmations": returned,
    }

def build_ui_semantic_validator_handoff_clearance_release_confirmation_board_latest_payload(
...
__all__ = [
    ...
    "build_ui_semantic_validator_handoff_clearance_release_confirmation_board_latest_payload",
]
