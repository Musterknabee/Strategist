"""Public payload builders for semantic validator handoff clearance release retention board."""
...
from strategy_validator.application.ui_semantic_validator_handoff_clearance_release_retention_board_rows import (
    _degraded,
    _matches,
    _row,
    _sort,
...
def _source_builder() -> SourceBuilder:
    ...
    from strategy_validator.application import ui_semantic_validator_handoff_clearance_release_retention_board as facade

    return facade.build_ui_semantic_validator_handoff_clearance_release_archive_board_payload


def build_ui_semantic_validator_handoff_clearance_release_retention_board_payload(
    *,
    ...
    release_retention_status: Iterable[str] | None = None,
    release_retention_readiness: Iterable[str] | None = None,
    release_archive_status: Iterable[str] | None = None,
    release_archive_readiness: Iterable[str] | None = None,
    ...
    source_rows = list(source_payload.get("release_archives") or [])
    ...
    kw = dict(
        ...
        release_retention_status=_norm_set(release_retention_status),
        release_retention_readiness=_norm_set(release_retention_readiness),
        release_archive_status=_norm_set(release_archive_status),
        release_archive_readiness=_norm_set(release_archive_readiness),
        ...
        ready_for_human_retention_observation=ready_for_human_retention_observation,
        blocked=blocked,
        waiting=waiting,
        requires_acceptance_observation=requires_acceptance_observation,
        requires_external_artifact=requires_external_artifact,
        requires_release_archive_review=requires_release_archive_review,
    )
    ...
    summary = {
        "source_release_archive_count_total": len(source_rows),
        "release_retention_count_total": len(rows),
        "release_retention_count_filtered": len(filtered),
        "release_retention_count_returned": len(returned),
        "fail_closed_count": sum(1 for row in filtered if row.get("release_retention_readiness") == "FAIL_CLOSED"),
        "waiting_count": sum(1 for row in filtered if row.get("release_retention_readiness") == "WAITING"),
        "human_retention_ready_observation_count": sum(
            1 for row in filtered if row.get("release_retention_readiness") == "HUMAN_RETENTION_READY_OBSERVATION"
        ),
        "ready_for_human_retention_observation_count": sum(
            1 for row in filtered if row.get("ready_for_human_retention_observation")
        ),
        ...
        "requires_release_archive_review_count": sum(1 for row in filtered if row.get("requires_release_archive_review")),
        "release_retention_write_allowed_count": 0,
        "release_retention_assertion_allowed_count": 0,
        "release_archive_write_allowed_count": 0,
        ...
        "latest_release_retention_id": None if latest is None else latest.get("release_retention_id"),
    }
    return {
        ...
        "filters": {},
        "summary": summary,
        "release_retention_status_counts": _counts(filtered, "release_retention_status"),
        "release_retention_readiness_counts": _counts(filtered, "release_retention_readiness"),
        "source_release_archive_status_counts": _counts(filtered, "source_release_archive_status"),
        "source_release_archive_readiness_counts": _counts(filtered, "source_release_archive_readiness"),
        ...
        "guardrails": [
            "read_plane_only_clearance_release_retention_no_retention_archive_completion_confirmation_acknowledgment_receipt_custody_transfer_handoff_packet_release_acceptance_signoff_approval_or_decision_is_written_here",
            "retention_cards_are_observations_derived_from_release_archives_not_operator_actions",
            "no_override_approval_signoff_validator_submission_adjudication_promotion_or_execution_authority",
        ],
        "routes": {
            "clearance_release_retention_board": "/ui/semantic-validator-handoff/clearance-release-retention-board",
            "clearance_release_archive_board": "/ui/semantic-validator-handoff/clearance-release-archive-board",
            "clearance_release_completion_board": "/ui/semantic-validator-handoff/clearance-release-completion-board",
        },
        "latest": latest,
        "release_retentions": returned,
    }


def build_ui_semantic_validator_handoff_clearance_release_retention_board_latest_payload(
    *, repo_root: str | Path | None = None, search_root: str | Path | None = None
) -> dict[str, Any]:
    return build_ui_semantic_validator_handoff_clearance_release_retention_board_payload(
...
__all__ = [
    ...
    "build_ui_semantic_validator_handoff_clearance_release_retention_board_latest_payload",
]
