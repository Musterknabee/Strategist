"""Shared helpers for semantic validator handoff clearance closeout board."""
...
_SCHEMA_VERSION = "ui_semantic_validator_handoff_clearance_closeout_board/v1"
_LIMIT_MAX = 1000
_STATUS_RANK = {
    "CLEARANCE_CLOSEOUT_BLOCKED": 0,
    "CLEARANCE_CLOSEOUT_WAITING_EXTERNAL_ARTIFACT": 1,
    "CLEARANCE_CLOSEOUT_WAITING_REFRESH": 2,
    "CLEARANCE_CLOSEOUT_WAITING_HUMAN_REVIEW": 3,
    "CLEARANCE_CLOSEOUT_INVESTIGATION_REQUIRED": 4,
    "CLEARANCE_CLOSEOUT_READY_FOR_AUTHORIZED_REVIEW": 5,
}
_READINESS_RANK = {"FAIL_CLOSED": 0, "WAITING": 1, "REVIEW_READY_OBSERVATION": 2}
...
def _best_priority(cards: list[dict[str, Any]]) -> str:
    return min((_s(card.get("priority")) or "P3" for card in cards), key=lambda value: _PRIORITY_RANK.get(_norm(value), 9), default="P3")

def _best_severity(cards: list[dict[str, Any]]) -> str:
    return min((_s(card.get("severity")) or "INFO" for card in cards), key=lambda value: _SEVERITY_RANK.get(_norm(value), 9), default="INFO")
...
def _authority() -> dict[str, Any]:
    return {
        ...
        "authority_boundary": "read_plane_only_clearance_closeout_visibility_no_closeout_write_no_assert_no_decision_no_approval_no_signoff_no_verify_write_no_completion_no_ack_no_repair_no_execute_no_attest_no_override_no_submit",
    }
