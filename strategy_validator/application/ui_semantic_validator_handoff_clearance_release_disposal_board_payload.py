"""Public payload builders for semantic validator handoff clearance release disposal board."""
...
def _source_builder() -> SourceBuilder:
    ...
    from strategy_validator.application import ui_semantic_validator_handoff_clearance_release_disposal_board as facade

    return facade.build_ui_semantic_validator_handoff_clearance_release_disposition_board_payload


def build_ui_semantic_validator_handoff_clearance_release_disposal_board_payload(
    *,
    ...
    release_disposal_status: Iterable[str] | None = None,
    release_disposal_readiness: Iterable[str] | None = None,
    release_disposition_status: Iterable[str] | None = None,
    release_disposition_readiness: Iterable[str] | None = None,
    ...
    source_rows = list(source_payload.get("release_dispositions") or [])
    ...
    kw = dict(
        ...
        release_disposal_status=_norm_set(release_disposal_status),
        release_disposal_readiness=_norm_set(release_disposal_readiness),
        release_disposition_status=_norm_set(release_disposition_status),
        release_disposition_readiness=_norm_set(release_disposition_readiness),
        ...
        ready_for_human_disposal_observation=ready_for_human_disposal_observation,
        blocked=blocked,
        waiting=waiting,
        requires_acceptance_observation=requires_acceptance_observation,
        requires_external_artifact=requires_external_artifact,
        requires_release_disposition_review=requires_release_disposition_review,
    )
    filters = {
        "experiment_id_contains": experiment_id_contains,
        "issue_contains": issue_contains,
        "evidence_lane": sorted(kw["evidence_lane"]),
        "release_disposal_status": sorted(kw["release_disposal_status"]),
        "release_disposal_readiness": sorted(kw["release_disposal_readiness"]),
        "release_disposition_status": sorted(kw["release_disposition_status"]),
        "release_disposition_readiness": sorted(kw["release_disposition_readiness"]),
        "release_completion_status": sorted(kw["release_completion_status"]),
        "release_completion_readiness": sorted(kw["release_completion_readiness"]),
        "release_acknowledgment_status": sorted(kw["release_acknowledgment_status"]),
        "release_acknowledgment_readiness": sorted(kw["release_acknowledgment_readiness"]),
        "release_receipt_status": sorted(kw["release_receipt_status"]),
        "release_receipt_readiness": sorted(kw["release_receipt_readiness"]),
        "release_custody_status": sorted(kw["release_custody_status"]),
        "release_custody_readiness": sorted(kw["release_custody_readiness"]),
        "release_handoff_status": sorted(kw["release_handoff_status"]),
        "release_handoff_readiness": sorted(kw["release_handoff_readiness"]),
        "release_packet_status": sorted(kw["release_packet_status"]),
        "release_packet_readiness": sorted(kw["release_packet_readiness"]),
        "release_status": sorted(kw["release_status"]),
        "release_readiness": sorted(kw["release_readiness"]),
        "acceptance_status": sorted(kw["acceptance_status"]),
        "acceptance_readiness": sorted(kw["acceptance_readiness"]),
        "priority": sorted(kw["priority"]),
        "severity": sorted(kw["severity"]),
        "trust_banner": sorted(kw["trust_banner"]),
        "owner_hint": sorted(kw["owner_hint"]),
        "ready_for_human_disposal_observation": ready_for_human_disposal_observation,
        "blocked": blocked,
        "waiting": waiting,
        "requires_acceptance_observation": requires_acceptance_observation,
    ...
    filters["limit"] = capped
    returned = filtered[:capped]
    latest = returned[0] if returned else None
    summary = {
        "source_release_disposition_count_total": len(source_rows),
        "release_disposal_count_total": len(rows),
        "release_disposal_count_filtered": len(filtered),
        "release_disposal_count_returned": len(returned),
        "fail_closed_count": sum(1 for row in filtered if row.get("release_disposal_readiness") == "FAIL_CLOSED"),
        "waiting_count": sum(1 for row in filtered if row.get("release_disposal_readiness") == "WAITING"),
        "human_disposal_ready_observation_count": sum(
            1 for row in filtered if row.get("release_disposal_readiness") == "HUMAN_DISPOSAL_READY_OBSERVATION"
        ),
        "ready_for_human_disposal_observation_count": sum(
            1 for row in filtered if row.get("ready_for_human_disposal_observation")
        ),
        ...
        "requires_release_disposition_review_count": sum(1 for row in filtered if row.get("requires_release_disposition_review")),
        "release_disposal_write_allowed_count": 0,
        "release_disposal_assertion_allowed_count": 0,
        "release_disposition_write_allowed_count": 0,
        ...
        "latest_release_disposal_id": None if latest is None else latest.get("release_disposal_id"),
    }
    return {
        ...
        "filters": filters,
        "summary": summary,
        "release_disposal_status_counts": _counts(filtered, "release_disposal_status"),
        "release_disposal_readiness_counts": _counts(filtered, "release_disposal_readiness"),
        "source_release_disposition_status_counts": _counts(filtered, "source_release_disposition_status"),
        "source_release_disposition_readiness_counts": _counts(filtered, "source_release_disposition_readiness"),
        "source_release_completion_status_counts": _counts(filtered, "source_release_completion_status"),
        "source_release_completion_readiness_counts": _counts(filtered, "source_release_completion_readiness"),
        ...
        "guardrails": [
            "read_plane_only_clearance_release_disposal_no_disposal_disposition_retention_completion_confirmation_acknowledgment_receipt_custody_transfer_handoff_packet_release_acceptance_signoff_approval_or_decision_is_written_here",
            "disposal_cards_are_observations_derived_from_release_dispositions_not_operator_actions",
            "no_override_approval_signoff_validator_submission_adjudication_promotion_or_execution_authority",
        ],
        "routes": {
            "clearance_release_disposal_board": "/ui/semantic-validator-handoff/clearance-release-disposal-board",
            "clearance_release_disposition_board": "/ui/semantic-validator-handoff/clearance-release-disposition-board",
            "clearance_release_completion_board": "/ui/semantic-validator-handoff/clearance-release-completion-board",
        },
        "latest": latest,
        "release_disposals": returned,
    }


def build_ui_semantic_validator_handoff_clearance_release_disposal_board_latest_payload(
