"""Public payload builders for semantic validator handoff clearance release disposition board."""
...
def _source_builder() -> SourceBuilder:
    ...
    return facade.build_ui_semantic_validator_handoff_clearance_release_retention_board_payload


def build_ui_semantic_validator_handoff_clearance_release_disposition_board_payload(
    *,
    ...
    source_rows = list(source_payload.get("release_retentions") or [])
    ...
    kw = dict(
        ...
        ready_for_human_disposition_observation=ready_for_human_disposition_observation,
        blocked=blocked,
        waiting=waiting,
        requires_acceptance_observation=requires_acceptance_observation,
        requires_external_artifact=requires_external_artifact,
        requires_release_retention_review=requires_release_retention_review,
    )
    ...
    summary = {
        "source_release_retention_count_total": len(source_rows),
        "release_disposition_count_total": len(rows),
        "release_disposition_count_filtered": len(filtered),
        "release_disposition_count_returned": len(returned),
        "fail_closed_count": sum(1 for row in filtered if row.get("release_disposition_readiness") == "FAIL_CLOSED"),
        "waiting_count": sum(1 for row in filtered if row.get("release_disposition_readiness") == "WAITING"),
        "human_disposition_ready_observation_count": sum(
            1 for row in filtered if row.get("release_disposition_readiness") == "HUMAN_DISPOSITION_READY_OBSERVATION"
        ),
        "ready_for_human_disposition_observation_count": sum(
            1 for row in filtered if row.get("ready_for_human_disposition_observation")
        ),
        ...
        "requires_release_retention_review_count": sum(1 for row in filtered if row.get("requires_release_retention_review")),
        "release_disposition_write_allowed_count": 0,
        "release_disposition_assertion_allowed_count": 0,
        ...
        "latest_release_disposition_id": None if latest is None else latest.get("release_disposition_id"),
    }
    return {
        ...
        "filters": {},
        "summary": summary,
        "release_disposition_status_counts": _counts(filtered, "release_disposition_status"),
        "release_disposition_readiness_counts": _counts(filtered, "release_disposition_readiness"),
        "source_release_retention_status_counts": _counts(filtered, "source_release_retention_status"),
        "source_release_retention_readiness_counts": _counts(filtered, "source_release_retention_readiness"),
        ...
        "guardrails": [
            "read_plane_only_clearance_release_disposition_no_disposition_retention_completion_confirmation_acknowledgment_receipt_custody_transfer_handoff_packet_release_acceptance_signoff_approval_or_decision_is_written_here",
            "disposition_cards_are_observations_derived_from_release_retentions_not_operator_actions",
        ...
        "latest": latest,
        "release_dispositions": returned,
    }


def build_ui_semantic_validator_handoff_clearance_release_disposition_board_latest_payload(
