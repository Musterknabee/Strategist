"""Public payload builders for semantic validator handoff clearance release receipt board."""
...
from strategy_validator.application.ui_semantic_validator_handoff_clearance_release_receipt_board_rows import (
    _degraded,
    _matches,
    _row,
    _sort,
...
def _source_builder() -> SourceBuilder:
    ...
    from strategy_validator.application import ui_semantic_validator_handoff_clearance_release_receipt_board as facade

    return facade.build_ui_semantic_validator_handoff_clearance_release_custody_board_payload


def build_ui_semantic_validator_handoff_clearance_release_receipt_board_payload(
    *,
    ...
    filtered = [
        ...
        if _matches(
            ...
            ready_for_human_receipt_observation=ready_for_human_receipt_observation,
            blocked=blocked,
            waiting=waiting,
            requires_acceptance_observation=requires_acceptance_observation,
            requires_external_artifact=requires_external_artifact,
            requires_release_custody_review=requires_release_custody_review,
        )
    ]
    ...
    summary = {
        "source_release_custody_count_total": len(source_rows),
        "release_receipt_count_total": len(rows),
        "release_receipt_count_filtered": len(filtered),
        "release_receipt_count_returned": len(returned),
        "fail_closed_count": sum(1 for row in filtered if row.get("release_receipt_readiness") == "FAIL_CLOSED"),
        "waiting_count": sum(1 for row in filtered if row.get("release_receipt_readiness") == "WAITING"),
        "human_receipt_ready_observation_count": sum(1 for row in filtered if row.get("release_receipt_readiness") == "HUMAN_RECEIPT_READY_OBSERVATION"),
        "ready_for_human_receipt_observation_count": sum(1 for row in filtered if row.get("ready_for_human_receipt_observation")),
        ...
        "requires_release_custody_review_count": sum(1 for row in filtered if row.get("requires_release_custody_review")),
        "release_receipt_write_allowed_count": 0,
        "release_receipt_assertion_allowed_count": 0,
        ...
        "latest_release_receipt_id": None if latest is None else latest.get("release_receipt_id"),
    }
    return {
        ...
        "filters": {},
        "summary": summary,
        "release_receipt_status_counts": _counts(filtered, "release_receipt_status"),
        "release_receipt_readiness_counts": _counts(filtered, "release_receipt_readiness"),
        ...
        "guardrails": [
            "read_plane_only_clearance_release_receipt_no_receipt_record_custody_transfer_handoff_packet_release_acceptance_signoff_approval_or_decision_is_written_here",
            "receipt_cards_are_observations_derived_from_release_custodies_not_operator_actions",
        ...
        "latest": latest,
        "release_receipts": returned,
    }


def build_ui_semantic_validator_handoff_clearance_release_receipt_board_latest_payload(
    *,
    repo_root: str | Path | None = None,
    search_root: str | Path | None = None,
) -> dict[str, Any]:
...
__all__ = [
    ...
    "build_ui_semantic_validator_handoff_clearance_release_receipt_board_latest_payload",
]
