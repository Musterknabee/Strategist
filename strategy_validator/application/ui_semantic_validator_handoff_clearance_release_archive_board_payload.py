"""Public payload builders for semantic validator handoff clearance release archive board."""
...
from strategy_validator.application.ui_semantic_validator_handoff_clearance_release_archive_board_rows import (
    _degraded,
    _matches,
    _row,
    _sort,
...
def _source_builder() -> SourceBuilder:
    ...
    from strategy_validator.application import ui_semantic_validator_handoff_clearance_release_archive_board as facade

    return facade.build_ui_semantic_validator_handoff_clearance_release_closure_board_payload


def build_ui_semantic_validator_handoff_clearance_release_archive_board_payload(
    *,
    ...
    release_closure_status: Iterable[str] | None = None,
    release_closure_readiness: Iterable[str] | None = None,
    ...
    source_rows = list(source_payload.get("release_closures") or [])
    ...
    kw = dict(
        ...
        release_closure_status=_norm_set(release_closure_status),
        release_closure_readiness=_norm_set(release_closure_readiness),
        ...
        ready_for_human_archive_observation=ready_for_human_archive_observation,
        blocked=blocked,
        waiting=waiting,
        requires_acceptance_observation=requires_acceptance_observation,
        requires_external_artifact=requires_external_artifact,
        requires_release_closure_review=requires_release_closure_review,
    )
    ...
    summary = {
        "source_release_closure_count_total": len(source_rows),
        "release_archive_count_total": len(rows),
        "release_archive_count_filtered": len(filtered),
        "release_archive_count_returned": len(returned),
        "fail_closed_count": sum(1 for row in filtered if row.get("release_archive_readiness") == "FAIL_CLOSED"),
        "waiting_count": sum(1 for row in filtered if row.get("release_archive_readiness") == "WAITING"),
        "human_archive_ready_observation_count": sum(
            1 for row in filtered if row.get("release_archive_readiness") == "HUMAN_ARCHIVE_READY_OBSERVATION"
        ),
        "ready_for_human_archive_observation_count": sum(
            1 for row in filtered if row.get("ready_for_human_archive_observation")
        ),
        ...
        "requires_release_closure_review_count": sum(1 for row in filtered if row.get("requires_release_closure_review")),
        "release_archive_write_allowed_count": 0,
        "release_archive_assertion_allowed_count": 0,
        "release_closure_write_allowed_count": 0,
        ...
        "latest_release_archive_id": None if latest is None else latest.get("release_archive_id"),
    }
    return {
        ...
        "filters": {},
        "summary": summary,
        "release_archive_status_counts": _counts(filtered, "release_archive_status"),
        "release_archive_readiness_counts": _counts(filtered, "release_archive_readiness"),
        "source_release_closure_status_counts": _counts(filtered, "source_release_closure_status"),
        "source_release_closure_readiness_counts": _counts(filtered, "source_release_closure_readiness"),
        ...
        "guardrails": [
            "read_plane_only_clearance_release_archive_no_archive_closure_completion_confirmation_acknowledgment_receipt_custody_transfer_handoff_packet_release_acceptance_signoff_approval_or_decision_is_written_here",
            "archive_cards_are_observations_derived_from_release_closures_not_operator_actions",
        ...
        "routes": {
            ...
            "clearance_release_closure_board": "/ui/semantic-validator-handoff/clearance-release-closure-board",
            "clearance_release_completion_board": "/ui/semantic-validator-handoff/clearance-release-completion-board",
        },
        "latest": latest,
        "release_archives": returned,
    }


def build_ui_semantic_validator_handoff_clearance_release_archive_board_latest_payload(
...
__all__ = [
    "build_ui_semantic_validator_handoff_clearance_release_archive_board_payload",
    "build_ui_semantic_validator_handoff_clearance_release_archive_board_latest_payload",
]
