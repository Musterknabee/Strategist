"""Public payload builders for semantic validator handoff clearance release closure board."""
...
from strategy_validator.application.ui_semantic_validator_handoff_clearance_release_closure_board_rows import (
    _degraded,
    _matches,
    _row,
    _sort,
...
def _source_builder() -> SourceBuilder:
    ...
    from strategy_validator.application import ui_semantic_validator_handoff_clearance_release_closure_board as facade

    return facade.build_ui_semantic_validator_handoff_clearance_release_completion_board_payload


def build_ui_semantic_validator_handoff_clearance_release_closure_board_payload(
    *,
    ...
    source_rows = list(source_payload.get("release_completions") or [])
    ...
    kw = dict(
        ...
        ready_for_human_closure_observation=ready_for_human_closure_observation,
        blocked=blocked,
        waiting=waiting,
        requires_acceptance_observation=requires_acceptance_observation,
        requires_external_artifact=requires_external_artifact,
        requires_release_completion_review=requires_release_completion_review,
    )
    ...
    summary = {
        "source_release_completion_count_total": len(source_rows),
        "release_closure_count_total": len(rows),
        "release_closure_count_filtered": len(filtered),
        "release_closure_count_returned": len(returned),
        "fail_closed_count": sum(1 for row in filtered if row.get("release_closure_readiness") == "FAIL_CLOSED"),
        "waiting_count": sum(1 for row in filtered if row.get("release_closure_readiness") == "WAITING"),
        "human_closure_ready_observation_count": sum(
            1
            for row in filtered
            if row.get("release_closure_readiness") == "HUMAN_CLOSURE_READY_OBSERVATION"
        ),
        "ready_for_human_closure_observation_count": sum(
            1 for row in filtered if row.get("ready_for_human_closure_observation")
        ),
        ...
        "requires_release_completion_review_count": sum(1 for row in filtered if row.get("requires_release_completion_review")),
        "release_closure_write_allowed_count": 0,
        "release_closure_assertion_allowed_count": 0,
        ...
        "latest_release_closure_id": None if latest is None else latest.get("release_closure_id"),
    }
    return {
        ...
        "filters": {},
        "summary": summary,
        "release_closure_status_counts": _counts(filtered, "release_closure_status"),
        "release_closure_readiness_counts": _counts(filtered, "release_closure_readiness"),
        ...
        "guardrails": [
            "read_plane_only_clearance_release_closure_no_closure_completion_confirmation_acknowledgment_receipt_custody_transfer_handoff_packet_release_acceptance_signoff_approval_or_decision_is_written_here",
            "closure_cards_are_observations_derived_from_release_completions_not_operator_actions",
        ...
        "latest": latest,
        "release_closures": returned,
    }


def build_ui_semantic_validator_handoff_clearance_release_closure_board_latest_payload(
    *, repo_root: str | Path | None = None, search_root: str | Path | None = None
) -> dict[str, Any]:
    return build_ui_semantic_validator_handoff_clearance_release_closure_board_payload(
        repo_root=repo_root, search_root=search_root, limit=1
    )


__all__ = [
    "build_ui_semantic_validator_handoff_clearance_release_closure_board_payload",
    "build_ui_semantic_validator_handoff_clearance_release_closure_board_latest_payload",
]
