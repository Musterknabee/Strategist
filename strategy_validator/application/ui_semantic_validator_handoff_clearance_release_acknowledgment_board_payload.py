"""Public payload builders for semantic validator handoff clearance release acknowledgment board."""
...
from strategy_validator.application.ui_semantic_validator_handoff_clearance_release_acknowledgment_board_rows import (
    _degraded,
    _matches,
    _row,
    _sort,
...
def _source_builder() -> SourceBuilder:
    ...
    return facade.build_ui_semantic_validator_handoff_clearance_release_receipt_board_payload


def build_ui_semantic_validator_handoff_clearance_release_acknowledgment_board_payload(
    *,
    ...
    kw = dict(
        ...
        ready_for_human_acknowledgment_observation=ready_for_human_acknowledgment_observation,
        blocked=blocked,
        waiting=waiting,
        requires_acceptance_observation=requires_acceptance_observation,
        requires_external_artifact=requires_external_artifact,
        requires_release_receipt_review=requires_release_receipt_review,
    )
    ...
    summary = {
        "source_release_receipt_count_total": len(source_rows),
        "release_acknowledgment_count_total": len(rows),
        "release_acknowledgment_count_filtered": len(filtered),
        "release_acknowledgment_count_returned": len(returned),
        "fail_closed_count": sum(1 for row in filtered if row.get("release_acknowledgment_readiness") == "FAIL_CLOSED"),
        "waiting_count": sum(1 for row in filtered if row.get("release_acknowledgment_readiness") == "WAITING"),
        "human_acknowledgment_ready_observation_count": sum(
            1
            for row in filtered
            if row.get("release_acknowledgment_readiness") == "HUMAN_ACKNOWLEDGMENT_READY_OBSERVATION"
        ),
        "ready_for_human_acknowledgment_observation_count": sum(
            1 for row in filtered if row.get("ready_for_human_acknowledgment_observation")
        ),
        ...
        "requires_release_receipt_review_count": sum(1 for row in filtered if row.get("requires_release_receipt_review")),
        ...
        "latest_release_acknowledgment_id": None if latest is None else latest.get("release_acknowledgment_id"),
    }
    return {
        ...
        "filters": {},
        "summary": summary,
        "release_acknowledgment_status_counts": _counts(filtered, "release_acknowledgment_status"),
        "release_acknowledgment_readiness_counts": _counts(filtered, "release_acknowledgment_readiness"),
        ...
        "guardrails": [
            "read_plane_only_clearance_release_acknowledgment_no_acknowledgment_receipt_custody_transfer_handoff_packet_release_acceptance_signoff_approval_or_decision_is_written_here",
            "acknowledgment_cards_are_observations_derived_from_release_receipts_not_operator_actions",
        ...
        "latest": latest,
        "release_acknowledgments": returned,
    }


def build_ui_semantic_validator_handoff_clearance_release_acknowledgment_board_latest_payload(
    *, repo_root: str | Path | None = None, search_root: str | Path | None = None
) -> dict[str, Any]:
    return build_ui_semantic_validator_handoff_clearance_release_acknowledgment_board_payload(
