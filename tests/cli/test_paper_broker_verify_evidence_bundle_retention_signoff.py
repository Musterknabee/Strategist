...
def _timeline(tmp_path: Path, tracking_id: str = "track-retention-signoff-verification-cli") -> tuple[list[PaperExecutionTimelineEntry], PaperExecutionTimelineSummary]:
...
def test_verify_evidence_bundle_retention_signoff_cli_writes_verification(tmp_path: Path, capsys) -> None:
    ...
    latest_signoff_path, _, _ = write_paper_execution_evidence_bundle_retention_signoff_artifact(
        ...
        operator_id="jp",
        decision_note="retain externally",
    ...
    assert payload["retention_signoff_artifact_hash_valid"] is True
    assert payload["signoff_statement_hash_valid"] is True
    ...
    assert artifact["schema_version"] == "paper_execution_evidence_bundle_retention_signoff_verification/v1"
