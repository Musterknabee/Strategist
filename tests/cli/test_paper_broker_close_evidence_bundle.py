...
def _setup(tmp_path: Path) -> Path:
    ...
    for idx, stage in enumerate(_STAGES, start=1):
        ...
        timeline.append(PaperExecutionTimelineEntry(stage=stage, stage_order=idx * 10, tracking_id="track-cli-closure", generated_at_utc=f"2026-05-03T12:0{idx}:00+00:00", artifact_path=str(path), artifact_sha256=digest, status="OK", ok=True, trusted=True, summary_line=f"{stage} trusted"))  # type: ignore[arg-type]
...
def test_close_evidence_bundle_cli_writes_closure(tmp_path: Path, capsys) -> None:
    output_root = _setup(tmp_path)

    rc = paper_broker.main(["close-evidence-bundle", "--output-root", str(output_root)])

    assert rc == 0
    payload = json.loads(capsys.readouterr().out)
    assert payload["closure_status"] == "READY_FOR_OPERATOR_REVIEW"
    assert payload["trust_banner"] == "TRUSTED"
    assert payload["source_attestation_verification_status"] == "PASS"
