...
def _timeline(tmp_path: Path, tracking_id: str = "track-retention-cli") -> tuple[list[PaperExecutionTimelineEntry], PaperExecutionTimelineSummary]:
...
def test_receipt_evidence_bundle_retention_cli_writes_receipt(tmp_path: Path, capsys) -> None:
    output_root, latest_export_verification_path = _setup(tmp_path)

    rc = paper_broker.main(["receipt-evidence-bundle-retention", "--export-verification-artifact", str(latest_export_verification_path), "--output-root", str(output_root)])

    assert rc == 0
    payload = json.loads(capsys.readouterr().out)
    assert payload["retention_status"] == "READY_FOR_RETENTION"
    assert payload["trust_banner"] == "TRUSTED"
    assert payload["export_verification_artifact_hash_valid"] is True
    assert payload["retained_entry_count"] == 7
    assert payload["retained_ready_entry_count"] == 7
    artifact_path = Path(payload["artifact"])
    assert artifact_path.exists()
    artifact = json.loads(artifact_path.read_text(encoding="utf-8"))
    assert artifact["schema_version"] == "paper_execution_evidence_bundle_retention_receipt/v1"
