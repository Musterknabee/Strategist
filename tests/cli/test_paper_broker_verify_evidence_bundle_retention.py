...
def test_verify_evidence_bundle_retention_cli_writes_verification(tmp_path: Path, capsys) -> None:
    output_root, latest_retention_receipt_path = _setup(tmp_path)

    rc = paper_broker.main(["verify-evidence-bundle-retention", "--retention-receipt-artifact", str(latest_retention_receipt_path), "--output-root", str(output_root)])

    assert rc == 0
    payload = json.loads(capsys.readouterr().out)
    assert payload["verification_status"] == "PASS"
    assert payload["trust_banner"] == "TRUSTED"
    assert payload["retention_receipt_hash_valid"] is True
    assert payload["retention_index_hash_valid"] is True
    ...
    assert artifact["schema_version"] == "paper_execution_evidence_bundle_retention_verification/v1"
