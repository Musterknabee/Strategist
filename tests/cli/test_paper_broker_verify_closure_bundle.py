...
def _setup(tmp_path: Path) -> tuple[Path, Path]:
    ...
    for idx, stage in enumerate(_STAGES, start=1):
        ...
        timeline.append(PaperExecutionTimelineEntry(stage=stage, stage_order=idx * 10, tracking_id="track-cli-closure-verify", generated_at_utc=f"2026-05-03T12:0{idx}:00+00:00", artifact_path=str(path), artifact_sha256=digest, status="OK", ok=True, trusted=True, summary_line=f"{stage} trusted"))  # type: ignore[arg-type]
...
def test_verify_evidence_bundle_closure_cli_writes_verification(tmp_path: Path, capsys) -> None:
    output_root, latest_closure_path = _setup(tmp_path)

    rc = paper_broker.main(["verify-evidence-bundle-closure", "--closure-artifact", str(latest_closure_path), "--output-root", str(output_root)])

    assert rc == 0
    payload = json.loads(capsys.readouterr().out)
    assert payload["verification_status"] == "PASS"
    assert payload["trust_banner"] == "TRUSTED"
    assert payload["closure_artifact_hash_valid"] is True
    assert payload["source_bundle_hash_valid"] is True
    assert payload["source_attestation_verification_hash_valid"] is True
    artifact_path = Path(payload["artifact"])
    assert artifact_path.exists()
    artifact = json.loads(artifact_path.read_text(encoding="utf-8"))
    assert artifact["schema_version"] == "paper_execution_evidence_bundle_closure_verification/v1"
