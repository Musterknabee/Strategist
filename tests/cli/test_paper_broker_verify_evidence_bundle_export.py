...
def _setup(tmp_path: Path) -> tuple[Path, Path]:
    ...
    for idx, stage in enumerate(_STAGES, start=1):
        ...
        timeline.append(PaperExecutionTimelineEntry(stage=stage, stage_order=idx * 10, tracking_id="track-cli-export-verify", generated_at_utc=f"2026-05-03T12:0{idx}:00+00:00", artifact_path=str(path), artifact_sha256=digest, status="OK", ok=True, trusted=True, summary_line=f"{stage} trusted"))  # type: ignore[arg-type]
...
def test_verify_evidence_bundle_export_cli_writes_verification(tmp_path: Path, capsys) -> None:
    output_root, latest_export_manifest_path = _setup(tmp_path)

    rc = paper_broker.main(["verify-evidence-bundle-export", "--export-manifest-artifact", str(latest_export_manifest_path), "--output-root", str(output_root)])

    assert rc == 0
    payload = json.loads(capsys.readouterr().out)
    assert payload["verification_status"] == "PASS"
    assert payload["trust_banner"] == "TRUSTED"
    assert payload["export_manifest_hash_valid"] is True
    assert payload["export_index_hash_valid"] is True
    assert payload["recomputed_export_entry_count"] == 7
    assert payload["recomputed_export_digest_valid_entry_count"] == 7
    artifact_path = Path(payload["artifact"])
    assert artifact_path.exists()
    artifact = json.loads(artifact_path.read_text(encoding="utf-8"))
    assert artifact["schema_version"] == "paper_execution_evidence_bundle_export_verification/v1"
