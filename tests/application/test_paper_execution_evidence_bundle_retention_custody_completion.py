...
def _setup_acknowledgment_verification(tmp_path: Path) -> tuple[Path, Path]:
    ...
    return output_root, ack_verification_path


def test_retention_custody_completion_passes_and_surfaces(monkeypatch, tmp_path: Path) -> None:
    ...
    assert completion.trust_banner == "TRUSTED"
    assert completion.completed_by == "operator-a"

    _, _, verification = write_paper_execution_evidence_bundle_retention_custody_completion_verification_artifact(
    ...
    assert verification.custody_completion_statement_hash_valid is True

    cockpit = build_ui_paper_execution_cockpit_payload(repo_root=tmp_path)
    assert cockpit["summary"]["latest_evidence_bundle_retention_custody_completion_status"] == "COMPLETED"
    assert cockpit["latest_evidence_bundle_retention_custody_completion_verification"]["verification_status"] == "PASS"

    daily = build_ui_daily_operator_run_payload(repo_root=tmp_path, database_path=str(tmp_path / "ledger.sqlite3"))
    assert daily["summary"]["paper_execution_latest_bundle_retention_custody_completion_verified_count"] == 1


def test_retention_custody_completion_blocks_without_acknowledgment_verification(tmp_path: Path) -> None:
    output_root = tmp_path / "artifacts" / "paper_broker"
    _, _, completion = write_paper_execution_evidence_bundle_retention_custody_completion_artifact(output_root=output_root)
    assert completion.completion_status == "BLOCKED"
    assert "RETENTION_CUSTODY_ACKNOWLEDGMENT_VERIFICATION_ARTIFACT_MISSING_OR_UNREADABLE" in completion.blockers


def test_retention_custody_completion_verification_blocks_when_statement_tampered(tmp_path: Path) -> None:
    output_root, ack_verification_path = _setup_acknowledgment_verification(tmp_path)
    completion_path, _, _ = write_paper_execution_evidence_bundle_retention_custody_completion_artifact(
        retention_custody_acknowledgment_verification_artifact_path=ack_verification_path,
        output_root=output_root,
    )
    raw = json.loads(completion_path.read_text(encoding="utf-8"))
    raw["custody_completion_statement_sha256"] = "0" * 64
    raw["artifact_sha256"] = canonical_json_sha256({k: v for k, v in raw.items() if k != "artifact_sha256"})
    completion_path.write_text(json.dumps(raw, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    ...
    assert verification.custody_completion_statement_hash_valid is False
