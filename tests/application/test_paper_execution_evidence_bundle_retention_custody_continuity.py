...
def _setup_custody_audit_verification(tmp_path: Path) -> tuple[Path, Path]:
    ...
    latest_audit_verification_path, _, _ = write_paper_execution_evidence_bundle_retention_custody_audit_verification_artifact(
        retention_custody_audit_artifact_path=latest_audit_path,
        output_root=output_root,
        generated_at_utc=datetime(2026, 5, 3, 12, 32, tzinfo=timezone.utc),
    )
    return output_root, latest_audit_verification_path


def test_retention_custody_continuity_passes_and_surfaces(monkeypatch, tmp_path: Path) -> None:
    ...
    latest_path, history_path, artifact = write_paper_execution_evidence_bundle_retention_custody_continuity_artifact(
        ...
        output_root=output_root,
        attested_by="operator-jp",
        custody_location="vault-a",
        continuity_note="continuity reviewed",
        generated_at_utc=datetime(2026, 5, 3, 12, 33, tzinfo=timezone.utc),
    )

    assert latest_path.exists()
    assert history_path.exists()
    assert artifact.continuity_status == "CONTINUITY_ATTESTED"
    assert artifact.trust_banner == "TRUSTED"
    assert artifact.retention_custody_audit_verification_artifact_hash_valid is True
    assert artifact.retention_custody_audit_artifact_hash_valid is True
    assert artifact.custody_audit_statement_hash_valid is True
    assert artifact.recomputed_retention_entry_count == 7
    assert artifact.custody_continuity_id and artifact.custody_continuity_id.startswith("retention-custody-continuity:")
    assert artifact.execution_authority == "NONE"

    views = read_paper_execution_evidence_bundle_retention_custody_continuity_views(output_root=output_root)
    assert views and views[0].continuity_status == "CONTINUITY_ATTESTED"

    cockpit = build_ui_paper_execution_cockpit_payload(repo_root=tmp_path)
    assert cockpit["summary"]["latest_evidence_bundle_retention_custody_continuity_status"] == "CONTINUITY_ATTESTED"
    assert cockpit["latest_evidence_bundle_retention_custody_continuity"]["custody_location"] == "vault-a"

    daily = build_ui_daily_operator_run_payload(repo_root=tmp_path, database_path=str(tmp_path / "ledger.sqlite3"))
    assert daily["summary"]["paper_execution_latest_bundle_retention_custody_continuity_attested_count"] == 1


def test_retention_custody_continuity_blocks_when_audit_verification_failed(tmp_path: Path) -> None:
    output_root, latest_audit_verification_path = _setup_custody_audit_verification(tmp_path)
    raw = json.loads(latest_audit_verification_path.read_text(encoding="utf-8"))
    raw["verification_status"] = "FAIL"
    raw["blockers"] = ["SYNTHETIC_CUSTODY_AUDIT_VERIFICATION_FAILURE"]
    raw["artifact_sha256"] = canonical_json_sha256({k: v for k, v in raw.items() if k != "artifact_sha256"})
    latest_audit_verification_path.write_text(json.dumps(raw, indent=2, sort_keys=True) + "\n", encoding="utf-8")

    _, _, artifact = write_paper_execution_evidence_bundle_retention_custody_continuity_artifact(
        retention_custody_audit_verification_artifact_path=latest_audit_verification_path,
        output_root=output_root,
    )

    assert artifact.continuity_status == "BLOCKED"
    assert artifact.trust_banner == "UNTRUSTED"
    assert "RETENTION_CUSTODY_AUDIT_VERIFICATION_NOT_PASS" in artifact.blockers
