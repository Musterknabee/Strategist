...
def _timeline(tmp_path: Path, tracking_id: str = "track-retention") -> tuple[list[PaperExecutionTimelineEntry], PaperExecutionTimelineSummary]:
...
def _setup_export_verification(tmp_path: Path) -> tuple[Path, Path]:
    ...
    return output_root, latest_export_verification_path


def test_retention_receipt_is_ready_and_surfaces(monkeypatch, tmp_path: Path) -> None:
    ...
    output_root, latest_export_verification_path = _setup_export_verification(tmp_path)

    latest_path, history_path, artifact = write_paper_execution_evidence_bundle_retention_receipt_artifact(export_verification_artifact_path=latest_export_verification_path, output_root=output_root, generated_at_utc=datetime(2026, 5, 3, 12, 19, tzinfo=timezone.utc))

    assert latest_path.exists()
    assert history_path.exists()
    assert artifact.retention_status == "READY_FOR_RETENTION"
    assert artifact.trust_banner == "TRUSTED"
    assert artifact.export_verification_artifact_hash_valid is True
    assert artifact.retained_entry_count == 7
    assert artifact.retained_ready_entry_count == 7
    assert artifact.total_size_bytes > 0
    assert artifact.retention_index_sha256
    assert all(entry.ready_for_retention for entry in artifact.entries)

    views = read_paper_execution_evidence_bundle_retention_receipt_views(output_root=output_root)
    assert views and views[0].retention_status == "READY_FOR_RETENTION"

    cockpit = build_ui_paper_execution_cockpit_payload(repo_root=tmp_path)
    assert cockpit["summary"]["latest_evidence_bundle_retention_receipt_status"] == "READY_FOR_RETENTION"
    assert cockpit["latest_evidence_bundle_retention_receipt"]["trust_banner"] == "TRUSTED"
    ...
    assert paper["summary"]["latest_evidence_bundle_retention_receipt_status"] == "READY_FOR_RETENTION"
    assert daily["summary"]["paper_execution_latest_bundle_retention_ready_count"] == 1


def test_retention_receipt_blocks_when_retained_file_tampered(tmp_path: Path) -> None:
    output_root, latest_export_verification_path = _setup_export_verification(tmp_path)
    export_verification_raw = json.loads(latest_export_verification_path.read_text(encoding="utf-8"))
    bundle_entry = next(entry for entry in export_verification_raw["entries"] if entry["kind"] == "bundle")
    bundle_path = Path(bundle_entry["artifact_path"])
    bundle_path.write_text(bundle_path.read_text(encoding="utf-8") + "\n", encoding="utf-8")

    _, _, artifact = write_paper_execution_evidence_bundle_retention_receipt_artifact(export_verification_artifact_path=latest_export_verification_path, output_root=output_root)

    assert artifact.retention_status == "BLOCKED"
    assert artifact.trust_banner == "UNTRUSTED"
    assert "RETENTION_ENTRY_FILE_DIGEST_MISMATCH:bundle" in artifact.blockers
    assert "RETENTION_ENTRY_SIZE_MISMATCH:bundle" in artifact.blockers
