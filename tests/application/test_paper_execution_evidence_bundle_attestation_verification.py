...
def _timeline(tmp_path: Path, tracking_id: str = "track-attest-verify") -> tuple[list[PaperExecutionTimelineEntry], PaperExecutionTimelineSummary]:
...
def _attested_bundle(tmp_path: Path) -> tuple[Path, Path]:
...
def test_attestation_verification_passes_and_surfaces(monkeypatch, tmp_path: Path) -> None:
    ...
    output_root, latest_attestation_path = _attested_bundle(tmp_path)

    latest_path, history_path, verification = write_paper_execution_evidence_bundle_attestation_verification_artifact(
        attestation_artifact_path=latest_attestation_path,
        output_root=output_root,
        generated_at_utc=datetime(2026, 5, 3, 12, 14, tzinfo=timezone.utc),
    )
    ...
    assert verification.statement_payload_hash_valid is True
    assert verification.envelope_payload_hash_valid is True
    assert verification.keyless_stub_signature_valid is True
    ...
    views = read_paper_execution_evidence_bundle_attestation_verification_views(output_root=output_root)
    assert views and views[0].verification_status == "PASS"

    cockpit = build_ui_paper_execution_cockpit_payload(repo_root=tmp_path)
    assert cockpit["summary"]["evidence_bundle_attestation_verification_count"] == 1
    assert cockpit["latest_evidence_bundle_attestation_verification"]["verification_status"] == "PASS"
    ...
    assert paper["summary"]["latest_evidence_bundle_attestation_verification_status"] == "PASS"
    assert daily["summary"]["paper_execution_latest_bundle_attestation_verified_count"] == 1


def test_attestation_verification_fails_when_attestation_payload_is_tampered(tmp_path: Path) -> None:
    output_root, latest_attestation_path = _attested_bundle(tmp_path)
    raw = json.loads(latest_attestation_path.read_text(encoding="utf-8"))
    raw["envelope"]["statement"]["predicate"]["drift_status"] = "TAMPERED_AFTER_ATTESTATION"
    latest_attestation_path.write_text(json.dumps(raw, indent=2, sort_keys=True) + "\n", encoding="utf-8")

    _, _, verification = write_paper_execution_evidence_bundle_attestation_verification_artifact(
        attestation_artifact_path=latest_attestation_path,
        output_root=output_root,
    )
    ...
    assert "ATTESTATION_ARTIFACT_SHA256_MISMATCH" in verification.blockers
    assert "ATTESTATION_STATEMENT_PAYLOAD_SHA256_MISMATCH" in verification.blockers
    assert verification.artifact_hash_valid is False
    assert verification.statement_payload_hash_valid is False
