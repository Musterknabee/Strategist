...
def _setup_custody_review(tmp_path: Path) -> tuple[Path, Path]:
    output_root, latest_continuity_verification_path = _setup_custody_continuity_verification(tmp_path)
    latest_review_path, _, _ = write_paper_execution_evidence_bundle_retention_custody_review_artifact(
    ...
    return output_root, latest_review_path


def test_retention_custody_review_verification_passes_and_surfaces(monkeypatch, tmp_path: Path) -> None:
    ...
    output_root, latest_review_path = _setup_custody_review(tmp_path)

    latest_path, history_path, artifact = write_paper_execution_evidence_bundle_retention_custody_review_verification_artifact(
        retention_custody_review_artifact_path=latest_review_path,
        output_root=output_root,
    ...
    assert artifact.retention_custody_review_artifact_hash_valid is True
    assert artifact.custody_review_statement_hash_valid is True
    ...
    views = read_paper_execution_evidence_bundle_retention_custody_review_verification_views(output_root=output_root)
    assert views and views[0].verification_status == "PASS"

    cockpit = build_ui_paper_execution_cockpit_payload(repo_root=tmp_path)
    assert cockpit["summary"]["latest_evidence_bundle_retention_custody_review_verification_status"] == "PASS"
    assert cockpit["latest_evidence_bundle_retention_custody_review_verification"]["custody_review_statement_hash_valid"] is True

    daily = build_ui_daily_operator_run_payload(repo_root=tmp_path, database_path=str(tmp_path / "ledger.sqlite3"))
    assert daily["summary"]["paper_execution_latest_bundle_retention_custody_review_verified_count"] == 1


def test_retention_custody_review_verification_blocks_when_statement_tampered(tmp_path: Path) -> None:
    output_root, latest_review_path = _setup_custody_review(tmp_path)
    raw = json.loads(latest_review_path.read_text(encoding="utf-8"))
    raw["custody_review_statement_sha256"] = "0" * 64
    raw["artifact_sha256"] = canonical_json_sha256({k: v for k, v in raw.items() if k != "artifact_sha256"})
    latest_review_path.write_text(json.dumps(raw, indent=2, sort_keys=True) + "\n", encoding="utf-8")

    _, _, artifact = write_paper_execution_evidence_bundle_retention_custody_review_verification_artifact(
        retention_custody_review_artifact_path=latest_review_path,
        output_root=output_root,
    )
    ...
    assert artifact.custody_review_statement_hash_valid is False
