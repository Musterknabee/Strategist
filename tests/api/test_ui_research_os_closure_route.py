...
def test_research_os_closure_route_returns_200() -> None:
    client = TestClient(create_app())
    r = client.get("/ui/research-os/closure/latest")
    assert r.status_code == 200
    body = r.json()
    assert body["schema_version"] == "ui_research_os_closure/v1"
...
def test_ui_public_facade_lists_research_os_closure() -> None:
    inv = build_ui_public_facade_inventory()
    assert any(r["method"] == "GET" and r["path"] == "/ui/research-os/closure/latest" for r in inv["routes"])
