...
def test_research_os_briefing_route_returns_200() -> None:
    client = TestClient(create_app())
    r = client.get("/ui/research-os/briefing/latest")
    assert r.status_code == 200
    body = r.json()
    assert body["schema_version"] == "ui_research_os_briefing/v1"
...
def test_ui_public_facade_lists_research_os_briefing() -> None:
    inv = build_ui_public_facade_inventory()
    assert any(r["method"] == "GET" and r["path"] == "/ui/research-os/briefing/latest" for r in inv["routes"])
