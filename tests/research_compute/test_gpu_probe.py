...
def test_gpu_probe_torch_missing(monkeypatch) -> None:
    ...
    monkeypatch.setattr("strategy_validator.research_compute.gpu_probe._probe_nvidia_smi", lambda: {"gpu_hardware_detected": False})
    monkeypatch.setattr(importlib, "import_module", _boom)
    payload = probe_gpu_capability()
    assert payload["ok"] is True
    assert payload["gpu_available"] is False
    assert payload["gpu_hardware_detected"] is False
    assert payload["reason"] == "GPU_UNAVAILABLE_CPU_FALLBACK"
    assert payload.get("fallback_status") == "GPU_UNAVAILABLE_CPU_FALLBACK"
...
def test_gpu_probe_torch_cpu_only(monkeypatch) -> None:
    ...
    monkeypatch.setattr("strategy_validator.research_compute.gpu_probe._probe_nvidia_smi", lambda: {"gpu_hardware_detected": True, "nvidia_smi_devices": [{"name": "Mock RTX"}]})
    ...
    assert payload["gpu_available"] is False
    assert payload["gpu_hardware_detected"] is True
    assert payload["nvidia_smi_devices"][0]["name"] == "Mock RTX"
...
def test_gpu_probe_cuda_available(monkeypatch) -> None:
    ...
    monkeypatch.setattr("strategy_validator.research_compute.gpu_probe._probe_nvidia_smi", lambda: {"gpu_hardware_detected": True})
    monkeypatch.setattr(importlib, "import_module", lambda _name: fake_torch)
    payload = probe_gpu_capability()
