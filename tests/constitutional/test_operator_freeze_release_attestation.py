...
def test_operator_freeze_release_attestation_materializes_certified_release_boundary(tmp_path: Path) -> None:
    ...
    report = materialize_operator_freeze_release_attestation(build_operator_freeze_release_attestation_request(attestation_root=tmp_path/'attestation', board_label='ops_board', attested_at_utc=datetime(2026,4,15,10,0,tzinfo=UTC)), freeze_release_gate=gate, board_label='ops_board')
    assert report.schema_version == 'oracle_operator_freeze_release_attestation/v1'
    assert report.return_ready_count >= 1
    payload = json.loads(Path(report.summary_output_path).read_text(encoding='utf-8'))
    assert payload['attestation_count'] >= 1


@pytest.mark.constitutional
def test_oracle_operator_freeze_release_attestation_cli_emits_typed_report(tmp_path: Path) -> None:
    output_path = tmp_path / 'freeze_release_attestation_report.json'
    rc = main(['oracle-operator-freeze-release-attestation','--issued-at-utc','2026-04-15T10:00:00Z','--surface-label','status pack','--board-label','ops_board','--attestation-root',str(tmp_path/'attestation'),'--prior-reopen-count','work:status_pack:release_readiness=3','--output',str(output_path)])
    assert rc == 0
    payload = json.loads(output_path.read_text(encoding='utf-8'))
    assert payload['schema_version'] == 'oracle_operator_freeze_release_attestation/v1'
    assert payload['return_ready_count'] >= 1
