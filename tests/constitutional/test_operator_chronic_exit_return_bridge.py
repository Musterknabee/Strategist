...
def test_operator_chronic_exit_return_bridge_materializes_rejoin_bridge(tmp_path: Path) -> None:
    ...
    report = materialize_operator_chronic_exit_return_bridge(build_operator_chronic_exit_return_bridge_request(bridge_root=tmp_path/'bridge', board_label='ops_board', bridged_at_utc=datetime(2026,4,15,10,0,tzinfo=UTC)), chronic_exit_certification=certification, board_label='ops_board')
    assert report.schema_version == 'oracle_operator_chronic_exit_return_bridge/v1'
    assert report.authorized_bridge_count >= 1
    payload = json.loads(Path(report.summary_output_path).read_text(encoding='utf-8'))
    assert payload['bridge_count'] >= 1


@pytest.mark.constitutional
def test_oracle_operator_chronic_exit_return_bridge_cli_emits_typed_report(tmp_path: Path) -> None:
    output_path = tmp_path / 'bridge_report.json'
    rc = main(['oracle-operator-chronic-exit-return-bridge','--issued-at-utc','2026-04-15T10:00:00Z','--surface-label','status pack','--board-label','ops_board','--bridge-root',str(tmp_path/'bridge'),'--prior-reopen-count','work:status_pack:release_readiness=3','--output',str(output_path)])
    assert rc == 0
    payload = json.loads(output_path.read_text(encoding='utf-8'))
    assert payload['schema_version'] == 'oracle_operator_chronic_exit_return_bridge/v1'
    assert payload['authorized_bridge_count'] >= 1
