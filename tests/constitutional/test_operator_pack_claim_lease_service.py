...
def test_operator_pack_claim_lease_materializes_active_claim_from_workboard(tmp_path: Path) -> None:
    ...
    assert claim_lease.schema_version == 'oracle_operator_pack_claim_lease/v1'
    assert claim_lease.total_claim_lease_count == 1
    item = claim_lease.items[0]
    assert item.claim_state == 'CLAIM_ACTIVE'
    assert item.lease_state == 'LEASE_ACTIVE'
    assert item.lease_action == 'MAINTAIN_LEASE'


@pytest.mark.constitutional
def test_briefing_pack_markdown_renders_operator_pack_claim_leases(tmp_path: Path) -> None:
    older = _build_briefing_report(tmp_path, summary_line='Older trusted briefing', trust_status='TRUSTED').model_copy(update={'generated_at_utc': datetime(2026, 4, 14, 7, 0, tzinfo=timezone.utc)})
    ...
    assert '## Operator Pack Claim Leases' in rendered
    assert 'LEASE_PENDING_ACQUISITION' in rendered
    assert 'Newest restricted briefing' in rendered


@pytest.mark.constitutional
def test_oracle_operator_pack_claim_lease_cli_emits_payload(tmp_path: Path) -> None:
    ...
    output_path = tmp_path / 'ORACLE_OPERATOR_PACK_CLAIM_LEASE.json'
    exit_code = main([
        'oracle-operator-pack-claim-lease',
    ...
    assert payload['schema_version'] == 'oracle_operator_pack_claim_lease/v1'
    assert payload['total_claim_lease_count'] == 1
    assert payload['items'][0]['claim_state'] == 'CLAIM_PENDING'


@pytest.mark.constitutional
def test_status_pack_markdown_renders_operator_pack_claim_leases(tmp_path: Path) -> None:
    older = _build_briefing_report(tmp_path, summary_line='Older trusted briefing', trust_status='TRUSTED').model_copy(update={'generated_at_utc': datetime(2026, 4, 14, 7, 0, tzinfo=timezone.utc)})
