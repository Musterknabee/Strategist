...
def test_operator_restoration_audit_materializes_bundle(tmp_path: Path) -> None:
    ...
    report = materialize_operator_restoration_audit(
        build_operator_restoration_audit_request(
            audit_root=tmp_path / 'restoration_audit',
            board_label='ops_board',
            audited_at_utc=datetime(2026,4,15,16,0,tzinfo=UTC),
        ),
        operator_queue_query_result=run_operator_queue_query(governance_work_queue=queue_state),
        board_label='ops_board',
    )
    assert report.schema_version == 'oracle_operator_restoration_audit/v1'
    payload = json.loads(Path(report.summary_output_path).read_text(encoding='utf-8'))
    assert payload['schema_version'] == 'oracle_operator_restoration_audit/v1'


@pytest.mark.constitutional
def test_operator_control_plane_bundle_includes_restoration_audit(tmp_path: Path) -> None:
    governance_plane = assess_governance_plane(
        evidence_freshness_status='EVIDENCE_CURRENT',
        evidence_integrity_status='INTEGRITY_VERIFIED',
        evidence_coverage_status='COVERAGE_COMPLETE',
    ...
    assert 'restoration_audit' in bundle.bundle_sections
    payload = json.loads(Path(bundle.summary_output_path).read_text(encoding='utf-8'))
    assert 'return_monitoring' in payload and 'restoration_audit' in payload


@pytest.mark.constitutional
def test_oracle_operator_restoration_audit_cli_emits_report(tmp_path: Path) -> None:
    output_path = tmp_path / 'restoration_audit_report.json'
    rc = main([
        'oracle-operator-restoration-audit',
        '--issued-at-utc', '2026-04-15T16:00:00Z',
        '--surface-label', 'status pack',
        '--board-label', 'ops_board',
        '--audit-root', str(tmp_path / 'restoration_audit'),
