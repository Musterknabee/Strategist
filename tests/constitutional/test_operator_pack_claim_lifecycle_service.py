...
def test_operator_pack_claim_lifecycle_materializes_active_renewal_from_workboard(tmp_path: Path) -> None:
    ...
    lifecycle = materialize_operator_pack_claim_lifecycle(
        build_operator_pack_claim_lifecycle_request(
    ...
    assert lifecycle.schema_version == 'oracle_operator_pack_claim_lifecycle/v1'
    assert lifecycle.total_claim_lifecycle_count == 1
    item = lifecycle.items[0]
    assert item.lifecycle_state == 'ACTIVE_RELEASE_RECOMMENDED'
    assert item.renewal_action == 'RELEASE_CLAIM'
    assert item.expiry_action == 'RELEASE_OR_EXPIRE'


@pytest.mark.constitutional
def test_briefing_pack_markdown_renders_operator_pack_claim_lifecycles(tmp_path: Path) -> None:
    older = _build_briefing_report(tmp_path, summary_line='Older trusted briefing', trust_status='TRUSTED').model_copy(update={'generated_at_utc': datetime(2026, 4, 14, 7, 0, tzinfo=timezone.utc)})
    ...
    assert '## Operator Pack Claim Lifecycles' in rendered
    assert 'PENDING_ACQUISITION' in rendered or 'ACQUIRE_AND_RENEW' in rendered
    assert 'Newest restricted briefing' in rendered


@pytest.mark.constitutional
def test_oracle_operator_pack_claim_lifecycle_cli_emits_payload(tmp_path: Path) -> None:
    ...
    output_path = tmp_path / 'ORACLE_OPERATOR_PACK_CLAIM_LIFECYCLE.json'
    exit_code = main([
        'oracle-operator-pack-claim-lifecycle',
    ...
    assert payload['schema_version'] == 'oracle_operator_pack_claim_lifecycle/v1'
    assert payload['total_claim_lifecycle_count'] == 1
    assert payload['items'][0]['renewal_action'] in {'ACQUIRE_AND_RENEW', 'NO_RENEWAL_ACTION', 'RELEASE_CLAIM'}


@pytest.mark.constitutional
def test_status_pack_markdown_renders_operator_pack_claim_lifecycles(tmp_path: Path) -> None:
    older = _build_briefing_report(tmp_path, summary_line='Older trusted briefing', trust_status='TRUSTED').model_copy(update={'generated_at_utc': datetime(2026, 4, 14, 7, 0, tzinfo=timezone.utc)})
