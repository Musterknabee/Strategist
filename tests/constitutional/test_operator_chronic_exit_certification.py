...
def test_operator_chronic_exit_certification_materializes_safe_rejoin_certification(tmp_path: Path) -> None:
    ...
    report = materialize_operator_chronic_exit_certification(build_operator_chronic_exit_certification_request(certification_root=tmp_path/'certification', board_label='ops_board', certified_at_utc=datetime(2026,4,15,10,0,tzinfo=UTC)), freeze_release_attestation=attestation, board_label='ops_board')
    assert report.schema_version == 'oracle_operator_chronic_exit_certification/v1'
    assert report.certified_count >= 1
    payload = json.loads(Path(report.summary_output_path).read_text(encoding='utf-8'))
    assert payload['certification_count'] >= 1


@pytest.mark.constitutional
def test_oracle_operator_chronic_exit_certification_cli_emits_typed_report(tmp_path: Path) -> None:
    output_path = tmp_path / 'chronic_exit_certification_report.json'
    rc = main(['oracle-operator-chronic-exit-certification','--issued-at-utc','2026-04-15T10:00:00Z','--surface-label','status pack','--board-label','ops_board','--certification-root',str(tmp_path/'certification'),'--prior-reopen-count','work:status_pack:release_readiness=3','--output',str(output_path)])
    assert rc == 0
    payload = json.loads(output_path.read_text(encoding='utf-8'))
    assert payload['schema_version'] == 'oracle_operator_chronic_exit_certification/v1'
