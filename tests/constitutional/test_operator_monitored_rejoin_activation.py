...
def _chronic_policy(tmp_path: Path):
    ...
    return materialize_operator_monitored_rejoin_policy(build_operator_monitored_rejoin_policy_request(policy_root=tmp_path/'rejoin_policy', board_label='ops_board', evaluated_at_utc=datetime(2026,4,15,10,0,tzinfo=UTC)), chronic_exit_return_bridge=bridge, board_label='ops_board')


@pytest.mark.constitutional
def test_operator_monitored_rejoin_activation_materializes_activation_surface(tmp_path: Path) -> None:
    policy = _chronic_policy(tmp_path)
    report = materialize_operator_monitored_rejoin_activation(
        build_operator_monitored_rejoin_activation_request(activation_root=tmp_path/'activation', board_label='ops_board', activated_at_utc=datetime(2026,4,15,10,0,tzinfo=UTC)),
        monitored_rejoin_policy=policy,
        board_label='ops_board',
    )
    assert report.schema_version == 'oracle_operator_monitored_rejoin_activation/v1'
...
@pytest.mark.constitutional
def test_oracle_operator_monitored_rejoin_activation_cli_emits_typed_report(tmp_path: Path) -> None:
    output_path = tmp_path / 'activation_report.json'
    rc = main(['oracle-operator-monitored-rejoin-activation','--issued-at-utc','2026-04-15T10:00:00Z','--surface-label','status pack','--board-label','ops_board','--activation-root',str(tmp_path/'activation'),'--prior-reopen-count','work:status_pack:release_readiness=3','--output',str(output_path)])
    assert rc == 0
    payload = json.loads(output_path.read_text(encoding='utf-8'))
    assert payload['schema_version'] == 'oracle_operator_monitored_rejoin_activation/v1'
    assert payload['activated_count'] >= 1
