...
def test_governance_plane_exposes_canonical_blocking_and_restricted_dimensions() -> None:
    ...
    assert "SUPPORT_CHAIN_TRUST" in blocked.governance_plane_blocking_dimensions
    assert "REMEDIATION" in blocked.governance_plane_blocking_dimensions
    assert "READINESS" in blocked.governance_plane_blocking_dimensions
    assert "RELIANCE" in blocked.governance_plane_blocking_dimensions
    assert "PROPAGATION" in blocked.governance_plane_blocking_dimensions
    assert "AUTOMATION" in blocked.governance_plane_blocking_dimensions
    assert not blocked.governance_plane_restricted_dimensions
    ...
    assert not restricted.governance_plane_blocking_dimensions
    assert "SUPPORT_CHAIN_TRUST" in restricted.governance_plane_restricted_dimensions
    assert "REMEDIATION" in restricted.governance_plane_restricted_dimensions
    assert "READINESS" in restricted.governance_plane_restricted_dimensions
    assert "RELIANCE" in restricted.governance_plane_restricted_dimensions
    assert "ESCALATION" in restricted.governance_plane_restricted_dimensions
    assert "PROPAGATION" in restricted.governance_plane_restricted_dimensions
    assert "AUTOMATION" in restricted.governance_plane_restricted_dimensions


def test_attestation_persists_and_renders_governance_dimensions() -> None:
    attestation = build_oracle_morning_attestation(_payload())
    assert isinstance(attestation.governance_plane_blocking_dimensions, list)
    assert isinstance(attestation.governance_plane_restricted_dimensions, list)
    markdown = render_oracle_morning_attestation_markdown(attestation)
    assert "## Governance dimensions" in markdown
    assert "Blocking dimensions:" in markdown
    assert "Restricted dimensions:" in markdown
    for item in attestation.governance_plane_blocking_dimensions + attestation.governance_plane_restricted_dimensions:
