...
def test_operator_reentry_post_review_gate_materializes_bundle(tmp_path: Path) -> None:
    ...
    report = materialize_operator_reentry_post_review_gate(
        build_operator_reentry_post_review_gate_request(
            review_root=tmp_path / 'post_review',
    ...
    assert report.schema_version == 'oracle_operator_reentry_post_review_gate/v1'
    assert report.total_item_count >= 0
    payload = json.loads(Path(report.summary_output_path).read_text(encoding='utf-8'))
    assert 'return_authorized_count' in payload
    assert Path(report.markdown_output_path).exists()


@pytest.mark.constitutional
def test_operator_control_plane_bundle_includes_completion_attestation_and_post_review_gate(tmp_path: Path) -> None:
    governance_plane = assess_governance_plane(
        evidence_freshness_status='EVIDENCE_CURRENT',
        evidence_integrity_status='INTEGRITY_VERIFIED',
        evidence_coverage_status='COVERAGE_COMPLETE',
    ...
    assert 'reentry_completion_attestation' in bundle.bundle_sections
    assert 'reentry_post_review_gate' in bundle.bundle_sections
    payload = json.loads(Path(bundle.summary_output_path).read_text(encoding='utf-8'))
    assert 'reentry_completion_attestation' in payload
    assert 'reentry_post_review_gate' in payload


@pytest.mark.constitutional
def test_oracle_operator_reentry_post_review_gate_cli_emits_report(tmp_path: Path) -> None:
    output_path = tmp_path / 'post_review_report.json'
    review_root = tmp_path / 'post_review'
    rc = main([
        'oracle-operator-reentry-post-review-gate',
    ...
    assert payload['schema_version'] == 'oracle_operator_reentry_post_review_gate/v1'
    assert (review_root / 'ORACLE_OPERATOR_REENTRY_POST_REVIEW_GATE.json').exists()
