...
def test_operator_pack_handoff_materializes_accepted_state_from_workboard(tmp_path: Path) -> None:
    ...
    assert handoff.schema_version == 'oracle_operator_pack_handoff/v1'
    assert handoff.total_handoff_count == 1
    item = handoff.items[0]
    assert item.handoff_state == 'HANDOFF_ACCEPTED'
    assert item.acceptance_state == 'ACCEPTED'
    assert item.ack_owner_lane == 'research_ops'


@pytest.mark.constitutional
def test_briefing_pack_markdown_renders_operator_pack_handoffs(tmp_path: Path) -> None:
    older = _build_briefing_report(tmp_path, summary_line='Older trusted briefing', trust_status='TRUSTED').model_copy(update={'generated_at_utc': datetime(2026, 4, 14, 7, 0, tzinfo=timezone.utc)})
    ...
    assert '## Operator Pack Handoffs' in rendered
    assert 'HANDOFF_PENDING_ACKNOWLEDGEMENT' in rendered
    assert 'Newest restricted briefing' in rendered


@pytest.mark.constitutional
def test_oracle_operator_pack_handoff_cli_emits_payload(tmp_path: Path) -> None:
    ...
    output_path = tmp_path / 'ORACLE_OPERATOR_PACK_HANDOFF.json'
    exit_code = main([
        'oracle-operator-pack-handoff',
    ...
    assert payload['schema_version'] == 'oracle_operator_pack_handoff/v1'
    assert payload['total_handoff_count'] == 1
    assert payload['items'][0]['acceptance_state'] == 'PENDING'


@pytest.mark.constitutional
def test_status_pack_markdown_renders_operator_pack_handoffs(tmp_path: Path) -> None:
    older = _build_briefing_report(tmp_path, summary_line='Older trusted briefing', trust_status='TRUSTED').model_copy(update={'generated_at_utc': datetime(2026, 4, 14, 7, 0, tzinfo=timezone.utc)})
