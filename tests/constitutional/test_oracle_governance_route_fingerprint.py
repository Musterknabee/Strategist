...
def test_governance_route_vector_and_sha256_are_stable() -> None:
    ...
    assert result.governance_plane_route_vector
    assert f"queue_key={result.governance_plane_queue_key}" in result.governance_plane_route_vector
    assert f"review_target={result.governance_plane_review_target}" in result.governance_plane_route_vector
    assert f"priority={result.governance_plane_priority_band}" in result.governance_plane_route_vector
    assert result.governance_plane_route_sha256 == hashlib.sha256(
        result.governance_plane_route_vector.encode("utf-8")
    ).hexdigest()


def test_oracle_attestation_carries_governance_route_fingerprint() -> None:
    ...
    assert attestation.governance_plane_route_vector
    assert attestation.governance_plane_route_sha256 == hashlib.sha256(
        attestation.governance_plane_route_vector.encode("utf-8")
    ).hexdigest()
    assert attestation.governance_plane_queue_key in attestation.governance_plane_route_vector
