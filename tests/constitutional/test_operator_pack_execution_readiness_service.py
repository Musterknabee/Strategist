...
def test_operator_pack_execution_readiness_materializes_execute_now_for_accepted_trusted_claim(tmp_path: Path) -> None:
    ...
    readiness = materialize_operator_pack_execution_readiness(
        build_operator_pack_execution_readiness_request(
    ...
    assert readiness.schema_version == 'oracle_operator_pack_execution_readiness/v1'
    assert readiness.total_execution_readiness_count == 1
    item = readiness.items[0]
    assert item.execution_posture == 'EXECUTE_NOW'
    assert item.readiness_state == 'READY_TO_EXECUTE'
    assert item.execution_action == 'EXECUTE_DECISION'


@pytest.mark.constitutional
def test_briefing_pack_markdown_renders_operator_pack_execution_readiness(tmp_path: Path) -> None:
    ...
    assert '## Operator Pack Execution Readiness' in rendered
    assert 'BLOCK_EXECUTION' in rendered or 'AWAIT_ACKNOWLEDGEMENT' in rendered or 'EXECUTE_NOW' in rendered
    assert 'Newest restricted briefing' in rendered


@pytest.mark.constitutional
def test_oracle_operator_pack_execution_readiness_cli_emits_payload(tmp_path: Path) -> None:
    ...
    output_path = tmp_path / 'ORACLE_OPERATOR_PACK_EXECUTION_READINESS.json'
    exit_code = main([
        'oracle-operator-pack-execution-readiness',
    ...
    assert payload['schema_version'] == 'oracle_operator_pack_execution_readiness/v1'
    assert payload['total_execution_readiness_count'] == 1
    assert payload['items'][0]['execution_posture'] in {'EXECUTE_NOW', 'BLOCK_EXECUTION', 'AWAIT_ACKNOWLEDGEMENT'}


@pytest.mark.constitutional
def test_status_pack_markdown_renders_operator_pack_execution_readiness(tmp_path: Path) -> None:
    older = _build_briefing_report(tmp_path, summary_line='Older trusted briefing', trust_status='TRUSTED').model_copy(update={'generated_at_utc': datetime(2026, 4, 14, 7, 0, tzinfo=timezone.utc)})
    newer = _build_briefing_report(tmp_path, summary_line='Newest restricted briefing', trust_status='TRUST_RESTRICTED').model_copy(update={'generated_at_utc': datetime(2026, 4, 14, 8, 0, tzinfo=timezone.utc)})
    materialize_briefing_pack_bundle(tmp_path / 'packs' / 'briefing_old', older, markdown='old', html='<p>old</p>')
    materialize_briefing_pack_bundle(tmp_path / 'packs' / 'briefing_new', newer, markdown='new', html='<p>new</p>')
