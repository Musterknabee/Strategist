...
def test_operator_monitored_rejoin_policy_materializes_typed_rejoin_posture(tmp_path: Path) -> None:
    ...
    report = materialize_operator_monitored_rejoin_policy(build_operator_monitored_rejoin_policy_request(policy_root=tmp_path/'policy', board_label='ops_board', evaluated_at_utc=datetime(2026,4,15,10,0,tzinfo=UTC)), chronic_exit_return_bridge=bridge, board_label='ops_board')
    assert report.schema_version == 'oracle_operator_monitored_rejoin_policy/v1'
    assert report.policy_count >= 1
    payload = json.loads(Path(report.summary_output_path).read_text(encoding='utf-8'))
    assert payload['standard_rejoin_count'] + payload['monitored_rejoin_count'] >= 1


@pytest.mark.constitutional
def test_oracle_operator_monitored_rejoin_policy_cli_emits_typed_report(tmp_path: Path) -> None:
    output_path = tmp_path / 'policy_report.json'
    rc = main(['oracle-operator-monitored-rejoin-policy','--issued-at-utc','2026-04-15T10:00:00Z','--surface-label','status pack','--board-label','ops_board','--policy-root',str(tmp_path/'policy'),'--prior-reopen-count','work:status_pack:release_readiness=3','--output',str(output_path)])
    assert rc == 0
    payload = json.loads(output_path.read_text(encoding='utf-8'))
    assert payload['schema_version'] == 'oracle_operator_monitored_rejoin_policy/v1'
