...
def test_operator_chronic_remediation_mandate_ledger_materializes_replayable_history(tmp_path: Path) -> None:
    ...
    report = materialize_operator_chronic_remediation_mandate_ledger(build_operator_chronic_remediation_mandate_ledger_request(ledger_root=tmp_path/'ledger', board_label='ops_board', mandated_at_utc=datetime(2026,4,15,10,0,tzinfo=UTC)), recurrence_tribunal_disposition=disposition, board_label='ops_board')
    assert report.schema_version == 'oracle_operator_chronic_remediation_mandate_ledger/v1'
    assert report.mandate_required_count >= 1
    payload = json.loads(Path(report.summary_output_path).read_text(encoding='utf-8'))
    assert payload['return_frozen_count'] >= 1


@pytest.mark.constitutional
def test_operator_control_plane_bundle_includes_chronic_mandate_layers(tmp_path: Path) -> None:
    governance_plane = assess_governance_plane(
        evidence_freshness_status='EVIDENCE_CURRENT', evidence_integrity_status='INTEGRITY_VERIFIED', evidence_coverage_status='COVERAGE_COMPLETE',
        support_verification_status='SUPPORT_VERIFIED', support_chain_trust_status='TRUSTED', support_chain_remediation_status='NO_REMEDIATION',
        support_chain_remediation_actions=[], operator_readiness='READY_FOR_REVIEW', surface_label='status pack')
    ...
    assert 'Tribunal dispositions:' in rendered
    assert 'Chronic remediation ledger:' in rendered
    payload = json.loads(Path(bundle.summary_output_path).read_text(encoding='utf-8'))
    assert 'recurrence_tribunal_disposition' in payload
    assert 'chronic_remediation_mandate_ledger' in payload


@pytest.mark.constitutional
def test_oracle_operator_chronic_remediation_mandate_ledger_cli_emits_typed_report(tmp_path: Path) -> None:
    output_path = tmp_path / 'ledger_report.json'
    rc = main(['oracle-operator-chronic-remediation-mandate-ledger','--issued-at-utc','2026-04-15T10:00:00Z','--surface-label','status pack','--board-label','ops_board','--ledger-root',str(tmp_path/'ledger'),'--prior-reopen-count','work:status_pack:release_readiness=3','--output',str(output_path)])
    assert rc == 0
    payload = json.loads(output_path.read_text(encoding='utf-8'))
    assert payload['schema_version'] == 'oracle_operator_chronic_remediation_mandate_ledger/v1'
