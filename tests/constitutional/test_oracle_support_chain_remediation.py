...
def test_support_chain_remediation_requires_repair_for_missing_and_unverified_support() -> None:
    status, _, actions = assess_support_chain_remediation(
        evidence_freshness_status="FRESH",
        stale_artifact_count=0,
        evidence_integrity_status="UNVERIFIED",
        unverified_artifact_count=1,
        evidence_coverage_status="MISSING",
        missing_expected_artifact_labels=["oracle_policy"],
        support_verification_status="UNVERIFIED",
        formal_verification_required=True,
    )
    assert status == "REMEDIATION_REQUIRED"
    assert any("missing support-chain artifacts" in action.lower() or "oracle_policy" in action for action in actions)
    assert any("verification" in action.lower() for action in actions)


def test_attestation_nominal_support_chain_requires_no_remediation() -> None:
    report = build_oracle_morning_attestation(_payload())
    assert report.support_chain_remediation_status == "NO_REMEDIATION"
    assert any("No support-chain remediation" in action for action in report.support_chain_remediation_actions)


def test_fusion_surfaces_recommended_remediation_when_formal_verification_is_absent(tmp_path: Path) -> None:
    ...
    assert report.support_chain_remediation_status == "REMEDIATION_RECOMMENDED"
    assert any("formal support verification" in action.lower() or "verification artifacts" in action.lower() for action in report.support_chain_remediation_actions)
    assert any("formal support verification" in action.lower() or "verification artifacts" in action.lower() for action in report.operator_actions)


def test_briefing_surfaces_required_remediation_for_missing_expected_support(tmp_path: Path) -> None:
    repo_root = tmp_path / "repo"
    artifacts_root = repo_root / "docs" / "artifacts"
    ...
    assert report.support_chain_remediation_status == "REMEDIATION_REQUIRED"
    assert any("refresh stale" in action.lower() or "verification" in action.lower() for action in report.support_chain_remediation_actions)
