...
def test_operator_escalation_packet_materializes_supervisor_ready_packets(tmp_path: Path) -> None:
    ...
    assert packet.schema_version == 'oracle_operator_escalation_packet/v1'
    assert packet.escalated_packet_count == 1
    assert packet.items[0].packet_status == 'SUPERVISOR_REVIEW_REQUIRED'
    assert len(packet.items[0].review_checklist) >= 3


@pytest.mark.constitutional
def test_oracle_operator_escalation_packet_cli_emits_typed_report(tmp_path: Path) -> None:
    output_path = tmp_path / 'ORACLE_OPERATOR_ESCALATION_PACKET_REPORT.json'
    rc = main([
        'oracle-operator-escalation-packet',
        '--issued-at-utc', '2026-04-15T10:00:00Z',
        '--surface-label', 'status pack',
        '--operator-readiness', 'HOLD_FOR_REFRESH',
        '--board-label', 'ops_board',
        '--packet-root', str(tmp_path / 'packet_bundle'),
    ...
    assert payload['schema_version'] == 'oracle_operator_escalation_packet/v1'
    assert payload['escalated_packet_count'] == 1
    assert payload['items'][0]['packet_status'] == 'SUPERVISOR_REVIEW_REQUIRED'


@pytest.mark.constitutional
def test_operator_control_plane_bundle_carries_escalation_packet(tmp_path: Path) -> None:
    governance_plane = assess_governance_plane(
        evidence_freshness_status='EVIDENCE_CURRENT',
        evidence_integrity_status='INTEGRITY_VERIFIED',
        evidence_coverage_status='COVERAGE_COMPLETE',
    ...
    assert 'escalation_packet' in bundle.bundle_sections
    assert bundle.escalation_packet['schema_version'] == 'oracle_operator_escalation_packet/v1'
    assert (tmp_path / 'bundle' / 'escalation_packet' / 'ORACLE_OPERATOR_ESCALATION_PACKET.json').exists()
