...
def test_operator_pack_dispatch_permission_materializes_dispatch_permitted_for_active_claim(tmp_path: Path) -> None:
    ...
    assert dispatch_permission.schema_version == 'oracle_operator_pack_dispatch_permission/v1'
    assert dispatch_permission.total_dispatch_permission_count == 1
    item = dispatch_permission.items[0]
    assert item.dispatch_permission == 'DISPATCH_PERMITTED'
    assert item.downstream_gate == 'ALLOW_DOWNSTREAM_ACTION'
    assert item.dispatch_action == 'AUTHORIZE_DISPATCH'


@pytest.mark.constitutional
def test_briefing_pack_markdown_renders_operator_pack_dispatch_permission(tmp_path: Path) -> None:
    ...
    assert '## Operator Pack Dispatch Permission' in rendered
    assert 'Newest restricted briefing' in rendered


@pytest.mark.constitutional
def test_oracle_operator_pack_dispatch_permission_cli_emits_payload(tmp_path: Path) -> None:
    ...
    output_path = tmp_path / 'ORACLE_OPERATOR_PACK_DISPATCH_PERMISSION.json'
    exit_code = main([
        'oracle-operator-pack-dispatch-permission',
    ...
    assert payload['schema_version'] == 'oracle_operator_pack_dispatch_permission/v1'
    assert payload['total_dispatch_permission_count'] == 1
    assert payload['items'][0]['dispatch_permission'] in {'DISPATCH_PERMITTED', 'DISPATCH_BLOCKED', 'DISPATCH_AWAITING_ACKNOWLEDGEMENT'}


@pytest.mark.constitutional
def test_status_pack_markdown_renders_operator_pack_dispatch_permission(tmp_path: Path) -> None:
    older = _build_briefing_report(tmp_path, summary_line='Older trusted briefing', trust_status='TRUSTED').model_copy(update={'generated_at_utc': datetime(2026, 4, 14, 7, 0, tzinfo=timezone.utc)})
    newer = _build_briefing_report(tmp_path, summary_line='Newest restricted briefing', trust_status='TRUST_RESTRICTED').model_copy(update={'generated_at_utc': datetime(2026, 4, 14, 8, 0, tzinfo=timezone.utc)})
    materialize_briefing_pack_bundle(tmp_path / 'packs' / 'briefing_old', older, markdown='old', html='<p>old</p>')
    materialize_briefing_pack_bundle(tmp_path / 'packs' / 'briefing_new', newer, markdown='new', html='<p>new</p>')
