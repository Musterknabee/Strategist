...
def test_operator_freeze_release_gate_materializes_release_decisions(tmp_path: Path) -> None:
    ...
    report = materialize_operator_freeze_release_gate(build_operator_freeze_release_gate_request(gate_root=tmp_path/'gate', board_label='ops_board', reviewed_at_utc=datetime(2026,4,15,10,0,tzinfo=UTC)), chronic_remediation_satisfaction=satisfaction, board_label='ops_board')
    assert report.schema_version == 'oracle_operator_freeze_release_gate/v1'
    assert report.release_authorized_count >= 1
    payload = json.loads(Path(report.summary_output_path).read_text(encoding='utf-8'))
    assert payload['gate_count'] >= 1


@pytest.mark.constitutional
def test_oracle_operator_freeze_release_gate_cli_emits_typed_report(tmp_path: Path) -> None:
    output_path = tmp_path / 'freeze_release_gate_report.json'
    rc = main(['oracle-operator-freeze-release-gate','--issued-at-utc','2026-04-15T10:00:00Z','--surface-label','status pack','--board-label','ops_board','--gate-root',str(tmp_path/'gate'),'--prior-reopen-count','work:status_pack:release_readiness=3','--output',str(output_path)])
    assert rc == 0
    payload = json.loads(output_path.read_text(encoding='utf-8'))
    assert payload['schema_version'] == 'oracle_operator_freeze_release_gate/v1'
    assert payload['release_authorized_count'] >= 1
