...
def test_operator_pack_approval_disposition_materializes_pending_signoff_for_active_claim(tmp_path: Path) -> None:
    ...
    workboard = OracleOperatorWorkboard(
        ...
        work_item_count=1,
        governed_work_item_count=1,
        journaled_work_item_count=0,
        ...
        entries=(
            OracleOperatorWorkboardEntry(
                ...
                source_created_at_utc=datetime(2026, 4, 14, 8, 30, tzinfo=timezone.utc),
    ...
    disposition = materialize_operator_pack_approval_disposition(
        build_operator_pack_approval_disposition_request(
            ...
            disposition_label_prefix='signoff-a',
        ),
        operator_workboard=workboard,
    )

    assert disposition.schema_version == 'oracle_operator_pack_approval_disposition/v1'
    assert disposition.total_disposition_count == 1
    item = disposition.items[0]
    assert item.disposition_posture == 'PENDING_SIGNOFF'
    assert item.signoff_state == 'SIGNOFF_PENDING'
    assert item.signoff_action == 'REQUEST_SIGNOFF'


@pytest.mark.constitutional
def test_briefing_pack_markdown_renders_operator_pack_approval_disposition(tmp_path: Path) -> None:
    older = _build_briefing_report(tmp_path, summary_line='Older trusted briefing', trust_status='TRUSTED').model_copy(update={'generated_at_utc': datetime(2026, 4, 14, 7, 0, tzinfo=timezone.utc)})
    ...
    assert '## Operator Pack Approval Disposition' in rendered
    assert 'Newest restricted briefing' in rendered


@pytest.mark.constitutional
def test_oracle_operator_pack_approval_disposition_cli_emits_payload(tmp_path: Path) -> None:
    ...
    output_path = tmp_path / 'ORACLE_OPERATOR_PACK_APPROVAL_DISPOSITION.json'
    exit_code = main([
        'oracle-operator-pack-approval-disposition',
        ...
        '--disposition-label-prefix', 'signoff-a',
        '--output', str(output_path),
    ])

    assert exit_code == 0
    payload = json.loads(output_path.read_text(encoding='utf-8'))
    assert payload['schema_version'] == 'oracle_operator_pack_approval_disposition/v1'
    assert payload['total_disposition_count'] == 1
    assert payload['items'][0]['disposition_posture'] in {'APPROVED', 'PENDING_SIGNOFF', 'DENIED'}


@pytest.mark.constitutional
def test_status_pack_markdown_renders_operator_pack_approval_disposition(tmp_path: Path) -> None:
    older = _build_briefing_report(tmp_path, summary_line='Older trusted briefing', trust_status='TRUSTED').model_copy(update={'generated_at_utc': datetime(2026, 4, 14, 7, 0, tzinfo=timezone.utc)})
    newer = _build_briefing_report(tmp_path, summary_line='Newest restricted briefing', trust_status='TRUST_RESTRICTED').model_copy(update={'generated_at_utc': datetime(2026, 4, 14, 8, 0, tzinfo=timezone.utc)})
    materialize_briefing_pack_bundle(tmp_path / 'packs' / 'briefing_old', older, markdown='old', html='<p>old</p>')
    materialize_briefing_pack_bundle(tmp_path / 'packs' / 'briefing_new', newer, markdown='new', html='<p>new</p>')
