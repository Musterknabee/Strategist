...
def test_operator_reentry_acknowledgement_timeout_materializes_timeout_and_escalation(tmp_path: Path) -> None:
    ...
    report = materialize_operator_reentry_acknowledgement_timeout(
    ...
    assert report.schema_version == 'oracle_operator_reentry_acknowledgement_timeout/v1'
    assert report.pending_item_count >= 1
    assert report.timed_out_count >= 1
    payload = json.loads(Path(report.summary_output_path).read_text(encoding='utf-8'))
    assert payload['items'][0]['timeout_status'] in {'TIMEOUT_BREACHED', 'DUE_SOON', 'WITHIN_TIMEOUT', 'NO_TIMEOUT_REQUIRED'}


@pytest.mark.constitutional
def test_oracle_operator_reentry_acknowledgement_timeout_cli_emits_typed_report(tmp_path: Path) -> None:
    output_path = tmp_path / 'ORACLE_OPERATOR_REENTRY_ACK_TIMEOUT_REPORT.json'
    rc = main([
        'oracle-operator-reentry-acknowledgement-timeout',
        '--issued-at-utc', '2026-04-15T11:30:00Z',
        '--accepted-at-utc', '2026-04-15T10:00:00Z',
        '--surface-label', 'status pack',
        '--board-label', 'ops_board',
        '--timeout-root', str(tmp_path / 'timeout'),
        '--output', str(output_path),
    ])
    assert rc == 0
    payload = json.loads(output_path.read_text(encoding='utf-8'))
    assert payload['schema_version'] == 'oracle_operator_reentry_acknowledgement_timeout/v1'
    assert (tmp_path / 'timeout' / 'ORACLE_OPERATOR_REENTRY_ACKNOWLEDGEMENT_TIMEOUT.json').exists()


@pytest.mark.constitutional
def test_control_plane_bundle_includes_reentry_acknowledgement_timeout(tmp_path: Path) -> None:
    governance_plane = assess_governance_plane(
        evidence_freshness_status='EVIDENCE_CURRENT',
        evidence_integrity_status='INTEGRITY_VERIFIED',
        evidence_coverage_status='COVERAGE_COMPLETE',
    ...
    assert 'reentry_acknowledgement_timeout' in bundle.bundle_sections
    assert payload['reentry_acknowledgement_timeout']['schema_version'] == 'oracle_operator_reentry_acknowledgement_timeout/v1'
    assert (bundle_root / 'reentry_acknowledgement_timeout' / 'ORACLE_OPERATOR_REENTRY_ACKNOWLEDGEMENT_TIMEOUT.json').exists()
