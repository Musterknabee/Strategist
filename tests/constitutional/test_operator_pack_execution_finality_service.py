...
def test_operator_pack_execution_finality_materializes(tmp_path: Path) -> None:
    ...
    finality = materialize_operator_pack_execution_finality(build_operator_pack_execution_finality_request(search_root=tmp_path, repo_root=tmp_path, current_pack_kind='briefing_pack', pack_kinds=('briefing_pack',), max_items=2), operator_workboard=_workboard())

    assert finality.items
    assert finality.items[0].terminal_decision in {'TERMINAL_EXECUTE', 'TERMINAL_FORCE_ELIGIBLE', 'TERMINAL_REJECT'}


@pytest.mark.constitutional
def test_oracle_operator_pack_execution_finality_cli_emits_payload(tmp_path: Path) -> None:
    older = _build_briefing_report(tmp_path, summary_line='Older restricted briefing', trust_status='TRUST_RESTRICTED').model_copy(update={'generated_at_utc': datetime(2026, 4, 14, 7, 0, tzinfo=timezone.utc)})
    newer = _build_briefing_report(tmp_path, summary_line='Newest restricted briefing', trust_status='TRUST_RESTRICTED').model_copy(update={'generated_at_utc': datetime(2026, 4, 14, 8, 0, tzinfo=timezone.utc)})
    materialize_briefing_pack_bundle(tmp_path / 'packs' / 'briefing_old', older, markdown='old', html='<p>old</p>')
    materialize_briefing_pack_bundle(tmp_path / 'packs' / 'briefing_new', newer, markdown='new', html='<p>new</p>')
    output_path = tmp_path / 'execution_finality.json'
    assert main(['oracle-operator-pack-execution-finality', '--search-root', str(tmp_path), '--repo-root', str(tmp_path), '--current-pack-kind', 'briefing_pack', '--pack-kind', 'briefing_pack', '--output', str(output_path)]) == 0
    payload = json.loads(output_path.read_text())
    assert payload['schema_version'] == 'oracle_operator_pack_execution_finality/v1'


@pytest.mark.constitutional
def test_render_oracle_briefing_pack_markdown_includes_execution_finality_section(tmp_path: Path) -> None:
    older = _build_briefing_report(tmp_path, summary_line='Older restricted briefing', trust_status='TRUST_RESTRICTED').model_copy(update={'generated_at_utc': datetime(2026, 4, 14, 7, 0, tzinfo=timezone.utc)})
    ...
    materialize_briefing_pack_bundle(tmp_path / 'packs' / 'briefing_old', older, markdown='old', html='<p>old</p>')
    materialize_briefing_pack_bundle(tmp_path / 'packs' / 'briefing_new', newer, markdown='new', html='<p>new</p>')
    assert '## Operator Pack Execution Finality' in render_oracle_briefing_pack_markdown(newer)


@pytest.mark.constitutional
def test_render_oracle_status_pack_markdown_includes_execution_finality_section(tmp_path: Path) -> None:
    older = _build_briefing_report(tmp_path, summary_line='Older restricted briefing', trust_status='TRUST_RESTRICTED').model_copy(update={'generated_at_utc': datetime(2026, 4, 14, 7, 0, tzinfo=timezone.utc)})
    newer = _build_briefing_report(tmp_path, summary_line='Newest restricted briefing', trust_status='TRUST_RESTRICTED').model_copy(update={'generated_at_utc': datetime(2026, 4, 14, 8, 0, tzinfo=timezone.utc)})
    materialize_briefing_pack_bundle(tmp_path / 'packs' / 'briefing_old', older, markdown='old', html='<p>old</p>')
    materialize_briefing_pack_bundle(tmp_path / 'packs' / 'briefing_new', newer, markdown='new', html='<p>new</p>')
    ...
    assert '## Operator Pack Execution Finality' in render_oracle_status_pack_markdown(report)
