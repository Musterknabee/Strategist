...
def test_operator_pack_claim_operability_materializes_operable_for_ready_claim(tmp_path: Path) -> None:
    ...
    operability = materialize_operator_pack_claim_operability(
        build_operator_pack_claim_operability_request(
            ...
            operability_label_prefix='oper-a',
        ),
        operator_workboard=workboard,
    )

    assert operability.schema_version == 'oracle_operator_pack_claim_operability/v1'
    assert operability.total_claim_operability_count == 1
    item = operability.items[0]
    assert item.operability_posture == 'CLAIM_OPERABLE'
    assert item.operability_state == 'CLAIM_OPERABILITY_READY'
    assert item.operability_action == 'OPERATE_UNDER_GOVERNED_LEASE'


@pytest.mark.constitutional
def test_oracle_operator_pack_claim_operability_cli_emits_payload(tmp_path: Path) -> None:
    ...
    output_path = tmp_path / 'ORACLE_OPERATOR_PACK_CLAIM_OPERABILITY.json'
    exit_code = main([
        'oracle-operator-pack-claim-operability',
        ...
        '--operability-label-prefix', 'oper-a',
        '--output', str(output_path),
    ])

    assert exit_code == 0
    payload = json.loads(output_path.read_text(encoding='utf-8'))
    assert payload['schema_version'] == 'oracle_operator_pack_claim_operability/v1'
    assert payload['total_claim_operability_count'] == 1
    assert payload['items'][0]['operability_posture'] in {'CLAIM_OPERABLE', 'CLAIM_CONSTRAINED', 'CLAIM_INOPERABLE'}


@pytest.mark.constitutional
def test_briefing_pack_markdown_renders_operator_pack_claim_operability(tmp_path: Path) -> None:
    ...
    assert '## Operator Pack Claim Operability' in rendered
    assert 'CLAIM_OPERABLE' in rendered or 'CLAIM_CONSTRAINED' in rendered or 'CLAIM_INOPERABLE' in rendered
    assert 'Newest restricted briefing' in rendered


@pytest.mark.constitutional
def test_status_pack_markdown_renders_operator_pack_claim_operability(tmp_path: Path) -> None:
    older = _build_briefing_report(tmp_path, summary_line='Older trusted briefing', trust_status='TRUSTED').model_copy(update={'generated_at_utc': datetime(2026, 4, 14, 7, 0, tzinfo=timezone.utc)})
    newer = _build_briefing_report(tmp_path, summary_line='Newest restricted briefing', trust_status='TRUST_RESTRICTED').model_copy(update={'generated_at_utc': datetime(2026, 4, 14, 8, 0, tzinfo=timezone.utc)})
    materialize_briefing_pack_bundle(tmp_path / 'packs' / 'briefing_old', older, markdown='old', html='<p>old</p>')
    materialize_briefing_pack_bundle(tmp_path / 'packs' / 'briefing_new', newer, markdown='new', html='<p>new</p>')
