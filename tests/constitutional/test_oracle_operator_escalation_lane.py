...
from strategy_validator.validator.oracle_operator_escalation import assess_operator_escalation_lane
...
def test_operator_escalation_lane_requires_constitutional_repair_for_untrusted_support() -> None:
    lane, _, reasons = assess_operator_escalation_lane(
    ...
    assert lane == "CONSTITUTIONAL_REPAIR_ESCALATION"
    assert any("repair" in reason.lower() or "untrusted" in reason.lower() for reason in reasons)


def test_attestation_surfaces_standard_operator_flow_for_nominal_inputs() -> None:
    report = build_oracle_morning_attestation(_payload())
    assert report.operator_escalation_lane == "STANDARD_OPERATOR_FLOW"
    assert "standard operator flow" in report.operator_escalation_summary_line.lower()
    assert report.operator_escalation_reasons


def test_fusion_surfaces_heightened_operator_escalation_when_verification_is_absent_for_complete_support(tmp_path: Path) -> None:
    ...
    assert report.operator_escalation_lane == "HEIGHTENED_OPERATOR_ESCALATION"
    assert any("restricted" in reason.lower() or "recommended" in reason.lower() or "cautious" in reason.lower() for reason in report.operator_escalation_reasons)
    assert any("heightened operator review lane" in action.lower() for action in report.operator_actions)


def test_briefing_surfaces_constitutional_repair_escalation_when_support_requires_repair(tmp_path: Path) -> None:
    repo_root = tmp_path / "repo"
    artifacts_root = repo_root / "docs" / "artifacts"
    ...
    assert report.operator_escalation_lane == "CONSTITUTIONAL_REPAIR_ESCALATION"
    assert any("repair" in reason.lower() or "untrusted" in reason.lower() for reason in report.operator_escalation_reasons)
    assert any("constitutional repair review" in action.lower() for action in report.operator_actions)
