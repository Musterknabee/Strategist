...
def test_attestation_persists_and_renders_governance_actions() -> None:
    attestation = build_oracle_morning_attestation(_payload())
    assert isinstance(attestation.governance_plane_actions, list)
    assert isinstance(attestation.governance_plane_action_items, list)
    assert len(attestation.governance_plane_actions) == len(set(attestation.governance_plane_actions))
    markdown = render_oracle_morning_attestation_markdown(attestation)
    assert "## Governance actions" in markdown
    for action in attestation.governance_plane_actions:
