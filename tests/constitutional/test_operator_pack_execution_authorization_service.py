...
def _active_workboard() -> OracleOperatorWorkboard:
...
def test_operator_pack_execution_authorization_materializes_authorized_for_active_claim(tmp_path: Path) -> None:
    ...
    authorization = materialize_operator_pack_execution_authorization(
        build_operator_pack_execution_authorization_request(
        ...
        operator_workboard=_active_workboard(),
    )

    assert authorization.items
    item = authorization.items[0]
    assert item.authorization_posture == 'AUTHORIZED'
    assert item.authority_state == 'EXECUTION_AUTHORIZED'
    assert item.authority_action == 'AUTHORIZE_EXECUTION'


@pytest.mark.constitutional
def test_oracle_operator_pack_execution_authorization_cli_emits_payload(tmp_path: Path) -> None:
    older = _build_briefing_report(tmp_path, summary_line='Older restricted briefing', trust_status='TRUST_RESTRICTED').model_copy(update={'generated_at_utc': datetime(2026, 4, 14, 7, 0, tzinfo=timezone.utc)})
    newer = _build_briefing_report(tmp_path, summary_line='Newest restricted briefing', trust_status='TRUST_RESTRICTED').model_copy(update={'generated_at_utc': datetime(2026, 4, 14, 8, 0, tzinfo=timezone.utc)})
    materialize_briefing_pack_bundle(tmp_path / 'packs' / 'briefing_old', older, markdown='old', html='<p>old</p>')
    materialize_briefing_pack_bundle(tmp_path / 'packs' / 'briefing_new', newer, markdown='new', html='<p>new</p>')
    ...
    exit_code = main([
        'oracle-operator-pack-execution-authorization',
    ...
    assert payload['schema_version'] == 'oracle_operator_pack_execution_authorization/v1'
    assert 'items' in payload


@pytest.mark.constitutional
def test_render_oracle_briefing_pack_markdown_includes_execution_authorization_section(tmp_path: Path) -> None:
    older = _build_briefing_report(tmp_path, summary_line='Older restricted briefing', trust_status='TRUST_RESTRICTED').model_copy(update={'generated_at_utc': datetime(2026, 4, 14, 7, 0, tzinfo=timezone.utc)})
    ...
    markdown = render_oracle_briefing_pack_markdown(newer)
    assert '## Operator Pack Execution Authorization' in markdown


@pytest.mark.constitutional
def test_render_oracle_status_pack_markdown_includes_execution_authorization_section(tmp_path: Path) -> None:
    older = _build_briefing_report(tmp_path, summary_line='Older restricted briefing', trust_status='TRUST_RESTRICTED').model_copy(update={'generated_at_utc': datetime(2026, 4, 14, 7, 0, tzinfo=timezone.utc)})
    newer = _build_briefing_report(tmp_path, summary_line='Newest restricted briefing', trust_status='TRUST_RESTRICTED').model_copy(update={'generated_at_utc': datetime(2026, 4, 14, 8, 0, tzinfo=timezone.utc)})
    materialize_briefing_pack_bundle(tmp_path / 'packs' / 'briefing_old', older, markdown='old', html='<p>old</p>')
    materialize_briefing_pack_bundle(tmp_path / 'packs' / 'briefing_new', newer, markdown='new', html='<p>new</p>')

    report = _status_report(tmp_path).model_copy(update={'search_root': str(tmp_path), 'repo_root': str(tmp_path), 'operator_workboard': _active_workboard()})
    markdown = render_oracle_status_pack_markdown(report)
    assert '## Operator Pack Execution Authorization' in markdown
