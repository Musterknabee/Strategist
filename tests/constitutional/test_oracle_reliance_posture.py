...
from strategy_validator.validator.oracle_reliance_posture import assess_operator_reliance_posture
...
def test_reliance_posture_escalates_to_repair_first_for_untrusted_support() -> None:
    posture, _, reasons = assess_operator_reliance_posture(
    ...
    assert posture == "REPAIR_FIRST"
    assert any("untrusted" in reason.lower() or "repair" in reason.lower() for reason in reasons)


def test_attestation_surfaces_routine_advisory_reliance_for_nominal_inputs() -> None:
    report = build_oracle_morning_attestation(_payload())
    assert report.operator_reliance_posture == "ROUTINE_ADVISORY"
    assert "routine advisory" in report.operator_reliance_summary_line.lower()
    assert report.operator_reliance_reasons


def test_fusion_surfaces_cautious_advisory_reliance_when_verification_is_absent_for_complete_support(tmp_path: Path) -> None:
    ...
    assert report.operator_reliance_posture == "CAUTIOUS_ADVISORY_ONLY"
    assert any("restricted" in reason.lower() or "recommended" in reason.lower() for reason in report.operator_reliance_reasons)


def test_briefing_surfaces_repair_first_reliance_when_support_requires_remediation(tmp_path: Path) -> None:
    repo_root = tmp_path / "repo"
    artifacts_root = repo_root / "docs" / "artifacts"
    ...
    assert report.operator_reliance_posture == "REPAIR_FIRST"
    assert any("repair" in reason.lower() or "untrusted" in reason.lower() for reason in report.operator_reliance_reasons)
