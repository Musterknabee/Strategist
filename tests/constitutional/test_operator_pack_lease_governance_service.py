...
def test_operator_pack_lease_governance_materializes_release_decision_for_degraded_active_claim(tmp_path: Path) -> None:
    ...
    newer = _build_briefing_report(tmp_path, summary_line='New restricted briefing', trust_status='TRUST_RESTRICTED').model_copy(update={'generated_at_utc': datetime(2026, 4, 14, 8, 0, tzinfo=timezone.utc)})
    ...
    governance = materialize_operator_pack_lease_governance(
        build_operator_pack_lease_governance_request(
    ...
    assert governance.schema_version == 'oracle_operator_pack_lease_governance/v1'
    assert governance.total_governance_count == 1
    item = governance.items[0]
    assert item.governance_decision == 'GOVERN_RELEASE'
    assert item.governance_action == 'RELEASE_LEASE'
    assert item.release_decision == 'RELEASE_OR_EXPIRE'


@pytest.mark.constitutional
def test_briefing_pack_markdown_renders_operator_pack_lease_governance(tmp_path: Path) -> None:
    older = _build_briefing_report(tmp_path, summary_line='Older trusted briefing', trust_status='TRUSTED').model_copy(update={'generated_at_utc': datetime(2026, 4, 14, 7, 0, tzinfo=timezone.utc)})
    ...
    assert '## Operator Pack Lease Governance' in rendered
    assert 'GOVERN_' in rendered
    assert 'Newest restricted briefing' in rendered


@pytest.mark.constitutional
def test_oracle_operator_pack_lease_governance_cli_emits_payload(tmp_path: Path) -> None:
    ...
    output_path = tmp_path / 'ORACLE_OPERATOR_PACK_LEASE_GOVERNANCE.json'
    exit_code = main([
        'oracle-operator-pack-lease-governance',
    ...
    assert payload['schema_version'] == 'oracle_operator_pack_lease_governance/v1'
    assert payload['total_governance_count'] == 1
    assert payload['items'][0]['governance_action'] in {'RELEASE_LEASE', 'RETAIN_AND_RENEW', 'ACQUIRE_PENDING_LEASE', 'ALLOW_EXPIRY', 'ACQUIRE_UNDER_ESCALATION'}


@pytest.mark.constitutional
def test_status_pack_markdown_renders_operator_pack_lease_governance(tmp_path: Path) -> None:
    older = _build_briefing_report(tmp_path, summary_line='Older trusted briefing', trust_status='TRUSTED').model_copy(update={'generated_at_utc': datetime(2026, 4, 14, 7, 0, tzinfo=timezone.utc)})
