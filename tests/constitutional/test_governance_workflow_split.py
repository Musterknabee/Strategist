...
def test_governance_workflow_implementations_live_in_bounded_modules() -> None:
    ...
    claim_foundations = Path("strategy_validator/control_plane/governance_claim_foundations.py").read_text(encoding="utf-8")
    ...
    assert "def build_governance_review_sort_key(" in claim_text
    assert "def materialize_governance_claim_envelope(" in claim_text
    assert "def _governance_claim_lease_action(" in claim_foundations
    assert "def _governance_claim_vector(" in claim_foundations
