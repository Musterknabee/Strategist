...
def test_operator_reopen_recurrence_policy_materializes_report(tmp_path: Path) -> None:
    ...
    lineage = materialize_operator_reopen_lineage(
        build_operator_reopen_lineage_request(
    ...
    report = materialize_operator_reopen_recurrence_policy(
        build_operator_reopen_recurrence_policy_request(
            policy_root=tmp_path / 'reopen_recurrence_policy',
            board_label='ops_board',
            evaluated_at_utc=datetime(2026,4,15,16,0,tzinfo=UTC),
            escalation_after_reopen_count=2,
            chronic_after_reopen_count=3,
        ),
        reopen_lineage=lineage,
        board_label='ops_board',
    )
    assert report.schema_version == 'oracle_operator_reopen_recurrence_policy/v1'
    payload = json.loads(Path(report.summary_output_path).read_text(encoding='utf-8'))
    assert payload['schema_version'] == 'oracle_operator_reopen_recurrence_policy/v1'
    if payload['items']:
        assert payload['items'][0]['recurrence_class'] in {'NON_RECURRENT', 'FIRST_REOPEN', 'REPEAT_REOPEN', 'CHRONIC_REOPEN'}


@pytest.mark.constitutional
def test_oracle_operator_reopen_recurrence_policy_cli_emits_report(tmp_path: Path) -> None:
    output_path = tmp_path / 'reopen_recurrence_policy_report.json'
    rc = main([
        'oracle-operator-reopen-recurrence-policy',
        '--issued-at-utc', '2026-04-15T16:00:00Z',
        '--surface-label', 'status pack',
        '--board-label', 'ops_board',
        '--policy-root', str(tmp_path / 'reopen_recurrence_policy'),
