...
def _activation(tmp_path: Path):
    ...
    return materialize_operator_monitored_rejoin_activation(build_operator_monitored_rejoin_activation_request(activation_root=tmp_path/'activation', board_label='ops_board', activated_at_utc=datetime(2026,4,15,10,0,tzinfo=UTC)), monitored_rejoin_policy=policy, board_label='ops_board')


@pytest.mark.constitutional
def test_operator_chronic_watch_handoff_materializes_monitoring_bridge(tmp_path: Path) -> None:
    activation = _activation(tmp_path)
    report = materialize_operator_chronic_watch_handoff(
        build_operator_chronic_watch_handoff_request(handoff_root=tmp_path/'handoff', board_label='ops_board', handed_off_at_utc=datetime(2026,4,15,10,0,tzinfo=UTC)),
        monitored_rejoin_activation=activation,
        board_label='ops_board',
    )
    assert report.schema_version == 'oracle_operator_chronic_watch_handoff/v1'
    assert report.monitoring_handoff_count >= 1
    payload = json.loads(Path(report.summary_output_path).read_text(encoding='utf-8'))
    assert payload['handoff_count'] >= 1


@pytest.mark.constitutional
def test_operator_control_plane_bundle_includes_monitored_rejoin_activation_and_watch_handoff(tmp_path: Path) -> None:
    governance_plane = assess_governance_plane(
        evidence_freshness_status='EVIDENCE_CURRENT', evidence_integrity_status='INTEGRITY_VERIFIED', evidence_coverage_status='COVERAGE_COMPLETE',
        support_verification_status='SUPPORT_VERIFIED', support_chain_trust_status='TRUSTED', support_chain_remediation_status='NO_REMEDIATION',
        support_chain_remediation_actions=[], operator_readiness='READY_FOR_REVIEW', surface_label='status pack')
    ...
    bundle = materialize_operator_control_plane_bundle(
        ...
        reopen_lineage_request=build_operator_reopen_lineage_request(lineage_root=tmp_path/'bundle'/'reopen_lineage', board_label='ops_board', analyzed_at_utc=datetime(2026,4,15,10,0,tzinfo=UTC), prior_reopen_counts={'work:status_pack:release_readiness': 3}),
    )
    rendered = '\n'.join(render_operator_control_plane_bundle_markdown_lines(bundle))
    assert bundle.monitored_rejoin_activation['activated_count'] >= 1
    assert bundle.chronic_watch_handoff['monitoring_handoff_count'] >= 1
    assert 'Monitored rejoin activation' in rendered
    assert 'Chronic watch handoff' in rendered


@pytest.mark.constitutional
def test_oracle_operator_chronic_watch_handoff_cli_emits_typed_report(tmp_path: Path) -> None:
    output_path = tmp_path / 'handoff_report.json'
    rc = main(['oracle-operator-chronic-watch-handoff','--issued-at-utc','2026-04-15T10:00:00Z','--surface-label','status pack','--board-label','ops_board','--handoff-root',str(tmp_path/'handoff'),'--prior-reopen-count','work:status_pack:release_readiness=3','--output',str(output_path)])
    assert rc == 0
    payload = json.loads(output_path.read_text(encoding='utf-8'))
    assert payload['schema_version'] == 'oracle_operator_chronic_watch_handoff/v1'
    assert payload['monitoring_handoff_count'] >= 1
